...
mod tests {
    ...
    const SECRET_CORPUS: &str =
        include_str!("../../../../fixtures/failure-corpora/v1/security-secrets.json");
    ...
    fn event(id: &str, secret: &str) -> RawEvent {
        RawEvent {
            ...
            payload: json!({
                ...
                "summary": format!("failed using {secret}")
            }),
        }
    }
    ...
    #[test]
    fn seeded_secret_plaintext_is_absent_from_database_and_batch() {
        let directory = TempDir::new().expect("temp");
        let path = directory.path().join("spool.sqlite3");
        let spool = open(&path, SpoolLimits::default());
        let secret = "sk-test_FAKE_51HACKATHON_NOT_A_REAL_KEY_000000";
        spool.append(&event("event-1", secret)).expect("append");
        let batch = spool.next_batch(1).expect("batch").expect("present");
        let batch_bytes = batch.serialized().expect("serialize");
        assert!(!String::from_utf8_lossy(&batch_bytes).contains(secret));
        drop(spool);
    ...
    #[test]
    fn complete_seeded_secret_corpus_is_absent_from_every_local_boundary() {
        let corpus: serde_json::Value =
            serde_json::from_str(SECRET_CORPUS).expect("secret corpus must parse");
        let secrets = corpus["fixtures"]
            .as_array()
            .expect("fixtures")
            .iter()
            .filter(|fixture| fixture["expected"]["secretDetected"] == true)
            .map(|fixture| {
        ...
        for (index, secret) in secrets.iter().enumerate() {
            let event_id = format!("secret-event-{index}");
            spool.append(&event(&event_id, secret)).expect("append");
            write_safe_log(
                &mut logs,
                spool.redactor(),
                LogLevel::Error,
                "fixture.secret",
                secret,
                &BTreeMap::from([("authorization".to_owned(), json!(secret))]),
            )
            .expect("safe log");
            let fingerprint = opaque_environment_fingerprint(&key(9, 1), secret);
        ...
        for secret in &secrets {
            assert!(!String::from_utf8_lossy(&logs).contains(secret));
            assert!(
                !String::from_utf8_lossy(&serialized).contains(secret),
                "serialized batch leaked corpus fixture beginning {:?}",
                secret.chars().take(24).collect::<String>()
            );
        }
        drop(spool);
                ...
                for secret in &secrets {
                    assert!(
                        !bytes
                            .windows(secret.len())
                            .any(|window| window == secret.as_bytes()),
                        "secret reached SQLite boundary"
                    );
                }
            }
        }
