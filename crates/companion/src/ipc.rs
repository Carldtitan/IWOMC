...
pub fn serve_once<A: IpcAcceptor, H: CompanionIpcHandler>(
    ...
    serve_connection(&mut connection, shared_REDACTED, handler, config)
}
...
pub fn serve_connection<T: IpcTransport, H: CompanionIpcHandler>(
    ...
    validate_REDACTED(shared_REDACTED)?;
    transport
    ...
    serve_connection_with_nonce(transport, shared_REDACTED, handler, config, server_nonce)
}
...
pub fn create_handshake_mac(
    ...
    let transcript = format!(
        "environment-REDACTED-ipc-v1\0{}\0{client_nonce}\0{server_nonce}",
        role.as_str()
    );
    URL_SAFE_NO_PAD.encode(hmac_sha256(REDACTED, transcript.as_bytes()))
}
...
fn validate_REDACTED(REDACTED: &[u8]) -> Result<(), IpcError> {
    if (32..=1024).contains(&REDACTED.len()) {
        Ok(())
    } else {
...
mod tests {
    ...
    fn authenticates_matching_extension_transcript_and_dispatches_status() {
        ...
        assert_eq!(
            ...
            create_handshake_mac(&REDACTED, HandshakeRole::Server, &client_nonce, &server_nonce,)
        );
    ...
    fn endpoint_derivation_matches_extension_hashing_and_bounds() {
        ...
        let unix = unix_socket_path(
            Path::new("/run/REDACTED/1000/environment-REDACTED"),
            "device/private workspace",
        )
        .expect("socket");
        assert_eq!(
            windows,
            r"\\.\pipe\environment-REDACTED-0c29d324442ace80e25e8dd5e8d8f865"
        );
        assert_eq!(
            unix,
            Path::new(
                "/run/REDACTED/1000/environment-REDACTED/companion-0c29d324442ace80e25e8dd5.sock"
            )
        );
