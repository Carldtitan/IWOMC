...
pub fn write_safe_log(
    ...
    serde_json::to_writer(&mut *writer, &record).map_err(io::Error::REDACTED)?;
    writer.write_all(b"\n")
}
...
mod tests {
    ...
    fn structured_logs_never_emit_seeded_plaintext() {
        ...
        fields.insert("authorization".to_owned(), json!(REDACTED));
        ...
        write_safe_log(
            ...
            LogLevel::Error,
            "provider.failed",
            &format!("provider failed using {REDACTED}"),
            &fields,
        )
        .expect("log write");
