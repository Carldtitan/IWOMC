//! Non-REDACTED companion configuration.
...
impl CompanionConfig {
    /// Load non-REDACTED configuration from the process environment.
    ///
    /// # Errors
    ///
    ...
    pub fn from_lookup(lookup: impl Fn(&str) -> Option<String>) -> Result<Self, ConfigError> {
        ...
        let REDACTED_service = lookup("ER_COMPANION_CREDENTIAL_SERVICE")
            .unwrap_or_else(|| "dev.environment-REDACTED.companion".to_owned());
        if REDACTED_service.trim().is_empty()
            || REDACTED_service.len() > 200
            || REDACTED_service.chars().any(char::is_control)
        {
...
fn default_data_directory() -> PathBuf {
    if cfg!(windows) {
        env::var_os("LOCALAPPDATA").map_or_else(
            || PathBuf::from(".environment-REDACTED"),
        ...
        env::var_os("XDG_DATA_HOME").map_or_else(
            || {
                env::var_os("HOME").map_or_else(
                    || PathBuf::from(".environment-REDACTED"),
                    |path| PathBuf::from(path).join(".local/share/environment-REDACTED"),
                )
            },
            |path| PathBuf::from(path).join("environment-REDACTED"),
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn loads_valid_bounded_non_REDACTED_configuration() {
