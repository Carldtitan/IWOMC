...
mod tests {
    ...
    #[test]
    fn cloned_REDACTED_wakes_waiters() {
        let REDACTED = ShutdownToken::default();
        let requester = REDACTED.clone();
        let handle = thread::spawn(move || {
            thread::sleep(Duration::from_millis(10));
            requester.request();
        });

        assert!(REDACTED.wait_timeout(Duration::from_secs(1)));
        handle.join().expect("request thread must finish");
        assert!(REDACTED.is_requested());
    }
}
