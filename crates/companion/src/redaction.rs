//! Local secret reduction and device-scoped pseudonymization.
...
use crate::{
    credentials::VersionedSecretKey,
...
impl RedactionCategory {
    const fn marker(self) -> &'static str {
        match self {
            ...
            Self::PasswordArgument => "<redacted:password>",
            Self::PemMaterial => "<redacted:pem-material>",
            Self::ProviderRawContent => "<redacted:raw-provider-content>",
            Self::RegistryCredential => "<redacted:registry-credential>",
            Self::UrlCredential => "<redacted:url-credentials>",
            Self::UserPattern => "<redacted:user-pattern>",
        }
    }
}
...
impl Redactor {
    ...
    /// # Errors
    ///
    /// Returns an error only when a built-in or user-defined expression cannot compile.
    pub fn new(
        identity_key: VersionedSecretKey,
        ...
        let built_in_patterns = [
            ...
            (
                ...
                "https://<redacted:url-credentials>@",
                RedactionCategory::UrlCredential,
            ),
            (
                r"(?i)(?://[^\s=]+/:_authToken\s*=\s*)[^\s]+",
                "//registry.invalid/:_authToken=<redacted:registry-credential>",
            ...
            (
                r"(?i)\b(?:api[_-]?key|access[_-]?token|auth[_-]?token|secret)\s*[:=]\s*[^\s,;]+",
                RedactionCategory::ApiKey.marker(),
                RedactionCategory::ApiKey,
            ),
        ]
    ...
    pub fn redact_text(&self, input: &str, maximum_bytes: usize) -> RedactedText {
        ...
        RedactedText {
            secret_detected: !categories.is_empty(),
            value,
            categories,
            truncated,
    ...
    pub fn environment_evidence(
        ...
        EnvironmentEvidence {
            ...
            changed,
            secret_detected: assessment
                .as_ref()
                .is_some_and(|candidate| candidate.secret_detected),
    ...
    fn redact_json_value(&self, key: Option<&str>, value: &Value) -> Value {
        ...
        if key.is_some_and(is_secret_field_name) {
...
mod tests {
    ...
    use crate::credentials::{SecretKey, VersionedSecretKey};
    ...
    #[test]
    fn redacts_supported_secret_classes_and_bounds_output() {
        let redactor = redactor();
        for secret in [
            "sk-test_FAKE_51HACKATHON_NOT_A_REAL_KEY_000000",
            "postgresql://user:pass@db.invalid/app",
            "--password=FakePassw0rd",
            "//registry.npmjs.org/:_authToken=npm_FAKE_TOKEN_1234567890123456",
            "SERVICE_TOKEN=fixture_secret_DO_NOT_USE_0123456789",
            "-----BEGIN PRIVATE KEY-----\nZmFrZQ==\n-----END PRIVATE KEY-----",
            "https://fixture-user:password@packages.invalid/index",
            "ACME-INTERNAL-42",
        ] {
            let result = redactor.redact_text(secret, 1024);
            assert!(result.secret_detected, "{secret}");
            assert!(!result.value.contains(secret), "{secret}");
        }
    ...
    fn environment_values_never_enter_evidence_but_can_be_compared() {
        let redactor = redactor();
        let first =
            redactor.environment_evidence("TOKEN", Some("fixture-secret"), "process", true, true);
        let second =
            redactor.environment_evidence("TOKEN", Some("fixture-secret"), "process", false, true);
    ...
    #[test]
    fn removes_raw_provider_fields_and_secret_named_fields() {
        let redactor = redactor();
        ...
        assert!(!serialized.contains("secret conversation"));
    ...
    fn pseudonyms_are_scoped_keyed_and_stable() {
        ...
        assert_ne!(
            repository,
            redactor.pseudonymize("username", "acme/private-repo")
        );
