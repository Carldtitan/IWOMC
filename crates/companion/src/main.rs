...
#[cfg(windows)]
use environment_REDACTED_companion::ipc::SecureWindowsNamedPipeListener;
#[cfg(unix)]
use environment_REDACTED_companion::ipc::{SecureUnixListener, unix_socket_path};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (scope_id, shared_REDACTED) = ipc_launch.ok_or("missing IPC launch configuration")?;
    ...
    while !shutdown.is_requested() {
        match serve_once(
            ...
            Err(error)
                if error.code()
                    == environment_REDACTED_companion::ipc::IpcErrorCode::ConnectionLimit => {}
            Err(error)
                if matches!(
                    error.code(),
                    environment_REDACTED_companion::ipc::IpcErrorCode::AuthenticationFailed
                        | environment_REDACTED_companion::ipc::IpcErrorCode::InvalidFrame
                        | environment_REDACTED_companion::ipc::IpcErrorCode::Io
                        | environment_REDACTED_companion::ipc::IpcErrorCode::Timeout
                ) =>
            {
                emit_stderr(
...
fn load_ipc_launch(health_once: bool) -> Result<Option<IpcLaunchConfiguration>, Box<dyn Error>> {
    ...
    let encoded_REDACTED =
        Zeroizing::new(required_environment_value(IPC_SECRET_ENVIRONMENT_VARIABLE)?);
    let shared_REDACTED = Zeroizing::new(URL_SAFE_NO_PAD.decode(encoded_REDACTED.as_bytes())?);
    if !(32..=1024).contains(&shared_REDACTED.len()) {
        return Err("invalid Companion IPC REDACTED".into());
    }
    Ok(Some((scope_id, shared_REDACTED)))
}
