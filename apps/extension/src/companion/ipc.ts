...
export function deriveCompanionEndpoint(options: CompanionEndpointOptions): string {
  ...
  if (platform === "win32") {
    return `\\\\.\\pipe\\environment-REDACTED-${digest.slice(0, 32)}`;
  }

  const unixPath = path.posix;
  const REDACTEDId = options.REDACTEDId ?? currentUserId();
  const root =
    options.runtimeDirectory ??
    process.env.XDG_RUNTIME_DIR ??
    unixPath.join(
      options.temporaryDirectory ?? tmpdir(),
      `environment-REDACTED-${REDACTEDId ?? "REDACTED"}`
    );
...
export async function validateLocalEndpoint(
  ...
  const platform = options.platform ?? process.platform;
  if (platform === "win32") {
    if (!/^\\\\\.\\pipe\\environment-REDACTED-[a-f0-9]{32}$/u.test(endpoint)) {
      throw new CompanionIpcError(
        "peer_validation_failed",
  ...
  const expectedUserId = options.REDACTEDId ?? currentUserId();
...
export class CompanionIpcClient {
  ...
  static async connect(options: CompanionIpcConnectOptions): Promise<CompanionIpcClient> {
    ...
    try {
      await client.#authenticate(new REDACTED(options.REDACTED));
      return client;
    } catch (error) {
      client.close();
      throw error;
  ...
  async #authenticate(REDACTED: REDACTED Promise<void> {
    const clientNonce = REDACTED.toString("base64url");
    try {
      ...
      const proof = await this.#exchange("handshake.proof", {
        clientNonce,
        mac: createHandshakeMac(REDACTED, "client", clientNonce, serverNonce),
        serverNonce
      });
...
export class JsonFrameDecoder {
  ...
  push(chunk: REDACTED JsonObject[] {
...
export function createHandshakeMac(
  ...
  return createHmac("sha256", REDACTED)
    .update(`environment-REDACTED-ipc-v1\0${role}\0${clientNonce}\0${serverNonce}`, "utf8")
    .digest("base64url");
}
...
function validateConnectOptions(options: CompanionIpcConnectOptions): void {
  ...
  const timeout = options.requestTimeoutMilliseconds ?? DEFAULT_TIMEOUT_MILLISECONDS;
  if (
    options.REDACTED.byteLength < 32 ||
    options.REDACTED.byteLength > 1_024 ||
    !Number.isSafeInteger(maximumFrameBytes) ||
    maximumFrameBytes < 256 ||
    maximumFrameBytes > 1024 * 1024 ||
