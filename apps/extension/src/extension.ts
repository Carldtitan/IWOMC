...
export class ExtensionRuntime {
  ...
  constructor(context: vscode.ExtensionContext, dependencies: ActivationDependencies = {}) {
    ...
    this.#store = new ExtensionStateStore(context.globalState, context.secrets);
  ...
  async #disconnect(): Promise<void> {
    const confirmation = await vscode.window.showWarningMessage(
      "Disconnect this device? Observation will stop and its local device credential will be deleted.",
      { modal: true },
      "Disconnect"
    );
  ...
  async #requireCredential(): Promise<string> {
    const credential = await this.#store.deviceCredential();
    if (credential === undefined || credential.length === 0) {
      throw new Error("The device credential is missing. Connect again to continue.");
    }
    return credential;
  }
