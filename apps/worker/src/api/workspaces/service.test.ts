...
class MemoryWorkspaceRepository implements WorkspaceRepository {
  ...
  membershipRole(workspaceId: string, userId: string): Promise<WorkspaceRole | undefined> {
    return Promise.resolve(this.memberships.get(`${workspaceId}:${userId}`));
  }
  ...
  consumeInvitationAndAddMember(
    ...
    this.memberships.set(`${invitation.workspaceId}:${userId}`, "member");
    return Promise.resolve(true);
  }
}

describe("WorkspaceService", () => {
  it("creates one idempotent personal workspace for a GitHub user", async () => {
    ...
    await expect(repository.membershipRole(first.workspaceId, "github-user-1")).resolves.toBe(
      "owner"
    );
  });
