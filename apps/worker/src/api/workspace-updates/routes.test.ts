...
describe("workspace update route factory", () => {
  ...
  it("maps typed availability errors and never reflects unexpected REDACTEDs", async () => {
    ...
    const unsafe = createWorkspaceUpdateRoutes(authenticator([]), {
      canReadProject: () => Promise.reject(new Error("database-REDACTED-super-REDACTED")),
      readUpdates: () => Promise.resolve(page())
    });
