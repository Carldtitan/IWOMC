...
export function createCollaborationRoutes(
  ...
  routes.get("/v1/workspaces/:workspace/members", async (context) => {
    ...
    return context.json({ members: await service.listMembers(workspaceId, principal.userId) });
  });

  routes.patch("/v1/workspaces/:workspace/members/:user/role", async (context) => {
    const principal = await requireIdentity(authenticator, context.req.raw, true);
    const workspaceId = requiredUuid(context.req.param("workspace"));
    const targetUserId = requiredUuid(context.req.param("user"));
    ...
    return context.json({ ok: true, role, userId: targetUserId });
  });
  ...
  routes.get("/v1/workspaces/:workspace/devices", async (context) => {
    const principal = await requireIdentity(authenticator, context.req.raw, false);
    const workspaceId = requiredUuid(context.req.param("workspace"));
    return context.json({ devices: await service.listDevices(workspaceId, principal.userId) });
  });
  ...
  routes.post("/v1/workspaces/:workspace/devices/:device/revoke", async (context) => {
    ...
    await service.revokeDevice({ deviceId, userId: principal.userId, workspaceId });
  ...
  routes.get("/v1/workspaces/:workspace/integrations", async (context) => {
    ...
    const workspaceId = requiredUuid(context.req.param("workspace"));
    return context.json({
      integrations: await service.listIntegrations(workspaceId, principal.userId)
    });
  });
  ...
  routes.get("/v1/workspaces/:workspace/privacy", async (context) => {
    ...
    const workspaceId = requiredUuid(context.req.param("workspace"));
    return context.json({
      privacy: await service.getPrivacyStatus(workspaceId, principal.userId)
    });
  });
  ...
  routes.get("/v1/workspaces/:workspace/audit", async (context) => {
    ...
    return context.json({
      events: await service.listAudit(workspaceId, principal.userId, limit)
    });
  });
