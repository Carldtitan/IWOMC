  ...
  async listMembers(workspaceId: string): Promise<readonly MemberView[]> {
    return this.#read(async (connection) => {
      const result = await connection.query<MemberRow>(
        `SELECT m.user_id, m.role, m.joined_at, u.display_name, u.github_login
    ...
    return this.#transaction(async (connection) => {
      ...
      const created = await connection.query<CommentRow>(
        `INSERT INTO comments (
           id, workspace_id, finding_id, author_user_id, body, body_digest
         ) VALUES ($1, $2, $3, $4, $5, $6)
  ...
  async listApprovals(
    ...
    return this.#read(async (connection) => {
      ...
      const result = await connection.query<ApprovalRow>(
        `SELECT id, recommendation_id, user_id, decision, reason_code, created_at
         FROM approvals
         WHERE workspace_id = $1
    ...
    return this.#transaction(async (connection) => {
      ...
      const inserted = await connection.query<ApprovalRow>(
        `INSERT INTO approvals (
           id, workspace_id, recommendation_id, user_id, decision,
           approval_policy_version, object_digest, reason_code
         ) VALUES ($1, $2, $3, $4, $5, 'mvp-v1', $6, $7)
         ON CONFLICT (workspace_id, recommendation_id, user_id) DO NOTHING
         RETURNING id, recommendation_id, user_id, decision, reason_code, created_at`,
        [
          input.approvalId,
          input.workspaceId,
  ...
  async listAudit(workspaceId: string, limit: number): Promise<readonly AuditView[]> {
    return this.#read(async (connection) => {
      const result = await connection.query<AuditRow>(
        `SELECT
           id, actor_type, actor_user_id, category, action, object_type,
...
function commentFromRow(row: CommentRow): CommentView {
  return {
    authorUserId: stringValue(row.author_user_id, "author user id"),
...
function auditFromRow(row: AuditRow): AuditView {
  return {
    ...
    actorUserId: nullableString(row.actor_user_id, "actor user id"),
