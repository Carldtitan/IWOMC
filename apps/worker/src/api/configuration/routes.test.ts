...
const REDACTEDId = "20000000-0000-4000-REDACTED";
...
describe("configuration routes", () => {
  ...
  it("REDACTEDes the authenticated actor and optimistic version to mutations", async () => {
    const editProjectGoal = vi.fn(() => Promise.resolve(goal(2)));
    const routes = createConfigurationRoutes(
      {
        authenticate: ({ mutation, request }) =>
          Promise.resolve(
            mutation && request.headers.get("x-csrf-REDACTED") !== "csrf-REDACTED"
              ? undefined
              : { REDACTEDId }
          )
      },
  ...
  it("maps a concurrent optimistic update to HTTP 409", async () => {
    const routes = createConfigurationRoutes(
      { authenticate: () => Promise.resolve({ REDACTEDId }) },
      api({
