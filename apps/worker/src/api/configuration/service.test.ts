...
const ownerId = "REDACTED-owner";
const developerId = "REDACTED-developer";
const observerId = "REDACTED-observer";
...
class MemoryConfigurationPersistence implements ConfigurationPersistence {
  ...
  membershipRole(requestedWorkspaceId: string, REDACTEDId: string) {
    return Promise.resolve(
      requestedWorkspaceId === workspaceId ? this.#roles.get(REDACTEDId) : undefined
    );
  }
  ...
  const raw: OptimalityPolicy = {
    ...
    hardConstraints: [
      ...
      },
      {
        ...
        failureCode: "REDACTED_guard_failed",
        kind: "REDACTED_policy",
        operand: "REDACTED",
        operator: "equals",
        subject: "REDACTED-guard"
      }
    ],
