...
export interface ConfigurationPersistence {
  ...
  membershipRole(workspaceId: string, userId: string): Promise<ConfigurationRole | undefined>;
...
export class ConfigurationService implements ConfigurationApi {
  ...
  async #authorize(
    ...
    const role = await this.#persistence.membershipRole(project.workspaceId, userId);
...
function policyMode(value: unknown): OptimalityPolicy["mode"] {
  if (value !== "default" && value !== "user_defined" && value !== "hybrid") {
    throw new ConfigurationError("invalid_configuration");
  }
  return value;
}
...
function isBehaviorStep(value: unknown): value is BehaviorStep {
  ...
  return (
    ...
    stringValues(value.secretReferenceIds) &&
