...
describe("integration status route factory", () => {
  ...
  it("maps typed not-found and availability failures without reflecting provider secrets", async () => {
    ...
    const unsafe = createIntegrationStatusRoutes(
      authenticator([]),
      authorizer([]),
      rejectingReader(new Error("postgres-password-and-braintrust-key"))
    );
  ...
  it("allowlists the response and cannot pass through undeclared sensitive properties", async () => {
    const unsafeStatus: RetryScheduledStatus & {
      ...
      ...status(),
      payloadObjectMetadataId: "object-secret",
      traceExport: {
        ...status().traceExport,
    ...
    expect(body).not.toContain("object-secret");
  });
