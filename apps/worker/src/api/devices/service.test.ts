...
class MemoryDeviceRepository implements DeviceRepository {
  readonly credentials = new Map<string, DeviceCredentialRecord>();
  ...
  findCredential(credentialId: string): Promise<DeviceCredentialRecord | undefined> {
    return Promise.resolve(this.credentials.get(credentialId));
  }
  ...
  revokeDevice(deviceId: string, revokedAtEpochSeconds: number): Promise<boolean> {
    ...
    for (const [id, credential] of this.credentials) {
      if (credential.deviceId === deviceId) {
        this.credentials.set(id, { ...credential, revokedAtEpochSeconds });
      }
    }
    return Promise.resolve(true);
  }
...
describe("DeviceService", () => {
  it("registers the public key and returns only one revocable product credential", async () => {
    ...
    expect(registered.credential).toMatch(/^er_device_v1\.[^.]+\.[A-Za-z0-9_-]+$/u);
    expect(fixture.repository.devices.get(registered.deviceId)?.publicSigningKey).toBe(
      fixture.publicKey
    );
    expect(fixture.repository.credentials.get(registered.credentialId)?.secretDigest).not.toContain(
      registered.credential
    );
    await expect(fixture.service.authenticate(registered.credential, 1_101)).resolves.toMatchObject(
      {
  ...
  it("rejects tampered, expired, and revoked device credentials", async () => {
    const fixture = await enrolledService();
    const registered = await fixture.service.register({
      code: fixture.code,
      devicePublicKey: fixture.publicKey,
    ...
    await expect(
      fixture.service.authenticate(`${registered.credential}tampered`, 1_101)
    ).rejects.toEqual(new DeviceCredentialError("invalid"));
    await expect(fixture.service.authenticate(registered.credential, 1_161)).rejects.toEqual(
    ...
    await expect(fixture.service.authenticate(registered.credential, 1_151)).rejects.toEqual(
