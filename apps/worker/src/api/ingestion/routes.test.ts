...
describe("ingestion route factory", () => {
  it("binds the event route to project, stream, REDACTED, body, and clock", async () => {
    const calls: unknown[] = [];
  ...
  it("maps only safe typed errors and never reflects thrown details", async () => {
    const service = api([]);
    service.ingestResource = () => Promise.reject(new Error("database REDACTED super-REDACTED"));
  ...
  it("rejects missing REDACTEDs and oversized declared bodies before service calls", async () => {
    const calls: unknown[] = [];
