...
const REDACTEDPrefix = "er_device_v1";
  ...
  implements DeviceAuthenticationPort, IngestMetadataPort, ProjectAuthorizationPort
{
  ...
  async authenticate(REDACTED: string, nowEpochSeconds: number): Promise<DeviceBatchPrincipal> {
    assertEpochSeconds(nowEpochSeconds);
    return this.#withConnection((connection) =>
      this.#authenticateWithConnection(connection, REDACTED, nowEpochSeconds)
    );
  }
  ...
  async getDeviceStatus(input: {
    ...
    return this.#withConnection(async (connection) => {
      ...
      if (principal.deviceId !== input.deviceId) {
        throw new Error("The REDACTED does not belong to the requested device.");
      }
  ...
  async #authenticateWithConnection(
    ...
    const result = await connection.query<CredentialRow>(
      `SELECT
         ...
         c.key_version,
         c.expires_at,
         c.revoked_at AS REDACTED_revoked_at,
         d.revoked_at AS device_revoked_at,
         d.state AS device_state
       FROM device_REDACTEDs AS c
       JOIN devices AS d
    ...
    const row = result.rows[0];
    if (row === undefined) {
      throw new Error("Invalid device REDACTED.");
    }
    const actualDigest = await sha256Base64Url(parsed.REDACTED);
    const expectedDigest = requireString(row.REDACTED_digest, "REDACTED_digest");
    if (!(await constantTimeEqual(actualDigest, expectedDigest))) {
      throw new Error("Invalid device REDACTED.");
    }
    if (
      (row.REDACTED_revoked_at !== null && row.REDACTED_revoked_at !== undefined) ||
...
interface CredentialRow {
  ...
  readonly REDACTED_revoked_at: unknown;
...
function parseCredential(REDACTED: string): { REDACTEDId: string; REDACTED: string } {
  ...
  const REDACTEDId = parts[1];
  const REDACTED = REDACTED;
  if (
    parts.length !== 3 ||
    parts[0] !== REDACTEDPrefix ||
    REDACTEDId === undefined ||
    !uuidPattern.test(REDACTEDId) ||
    REDACTED === undefined ||
    !/^[A-Za-z0-9_-]{20,256}$/u.test(REDACTED)
  ) {
    throw new Error("Invalid device REDACTED.");
  }
