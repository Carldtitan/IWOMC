...
const publicSigningKey = btoa(String.fromCharCode(...new Uint8Array(32).fill(9)));

describe("HyperdrivePostgresIngestionDriver", () => {
  it("authenticates a credential by digest and never sends its secret to PostgreSQL", async () => {
    const secret = "correct_device_secret_value_123";
    const connection = new ScriptedConnection(async (sql) => {
      if (sql.includes("FROM device_credentials AS c")) {
        return rows({
          credential_digest: await sha256Base64Url(secret),
    ...
    await expect(
      driver.authenticate(`er_device_v1.${credentialId}.${secret}`, 1_700_000_000)
    ).resolves.toEqual({
      deviceId,
      publicSigningKey,
      signingKeyVersion: 2,
    ...
    expect(JSON.stringify(connection.calls)).not.toContain(secret);
    expect(connection.calls[0]?.values).toEqual([credentialId]);
  });

  it("rejects the wrong credential secret before updating device activity", async () => {
    const connection = new ScriptedConnection(async (sql) => {
      if (!sql.includes("FROM device_credentials AS c")) {
        throw new Error(`Unexpected SQL: ${sql}`);
      }
      return rows({
        credential_digest: await sha256Base64Url("correct_device_secret_value_123"),
        credential_revoked_at: null,
        device_id: deviceId,
        device_revoked_at: null,
        device_state: "online",
    ...
    await expect(
      driverWith(connection).authenticate(
        `er_device_v1.${credentialId}.wrong_device_secret_value_12345`,
        1_700_000_000
      )
