...
const envelopeMagic = new Uint8Array([0x45, 0x52, 0x49, 0x47]);
...
export class CloudflarePayloadProtection implements PayloadProtectionPort {
  ...
  async protect(
    ...
    const bytes = concatenate([
      envelopeMagic,
      new Uint8Array([envelopeVersion]),
      nonce,
      ciphertext
    ]);
  ...
  async unprotect(envelope: Uint8Array, authenticatedMetadata: Uint8Array): Promise<Uint8Array> {
    if (
