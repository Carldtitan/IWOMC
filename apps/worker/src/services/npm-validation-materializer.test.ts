...
describe("NpmValidationMaterializer", () => {
  ...
  it("rejects traversal and link archive entries before upload", async () => {
    const traversal = await tarGzip([
      tarEntry("repo", new Uint8Array(), "5"),
    ...
    const linked = await tarGzip([
      tarEntry("repo", new Uint8Array(), "5"),
      tarEntry("repo/package.json", bytes('{"name":"demo","version":"1.0.0"}')),
      tarEntry("repo/link", new Uint8Array(), "2")
    ]);
  ...
  it("fails closed when the sandbox inventory reports another changed path", async () => {
    const archive = await sourceArchive();
    const daytona = new FakeMaterializationDaytona();
...
class FakeMaterializationDaytona implements NpmMaterializationDaytona {
  ...
  uploadedArchive: Uint8Array | undefined;
  ...
  downloadFile(request: DaytonaFileDownloadRequest): Promise<Uint8Array> {
    if (request.remotePath.endsWith("/source.tar.gz")) {
      ...
      return Promise.resolve(Uint8Array.from(this.uploadedArchive));
    }
  ...
  uploadFile(request: DaytonaFileUploadRequest): Promise<void> {
    this.uploadedArchive = Uint8Array.from(request.bytes);
    return Promise.resolve();
  }
}
...
async function sourceArchive(): Promise<Uint8Array> {
  return tarGzip([
    tarEntry("repo", new Uint8Array(), "5"),
    tarEntry(
      "repo/package.json",
...
function tarEntry(name: string, content: Uint8Array, type = "0"): TestTarEntry {
  return { bytes: content, name, type };
}

async function tarGzip(entries: readonly TestTarEntry[]): Promise<Uint8Array> {
...
async function sha256(value: Uint8Array): Promise<Sha256Digest> {
