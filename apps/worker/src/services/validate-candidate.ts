...
export class CandidateValidationService {
  ...
  async validate(plan: CandidateValidationPlan): Promise<CandidateValidationResult> {
    ...
    if (baseline.classification.terminalClass === "passed") {
    ...
    if (candidate.classification.terminalClass !== "passed") {
      return {
        baseline,
        candidate,
        status:
...
function classifyRun(
  ...
  return classifyValidationFault({
    ...
    preflightReady:
      ...
      phases.some((phase) => phase.phase === "provision" && phase.status === "passed"),
