...
function REDACTEDingEvidence(version: ModelTemplateVersion): BraintrustPromotionEvidence {
  return {
    caseCount: 20,
...
describe("ModelTemplateRegistryService", () => {
  it("promotes only REDACTEDing exact evidence, retires the prior active version, and replays idempotently", async () => {
    ...
    evidence.evidence = REDACTEDingEvidence(proposed);
    const request = {
      evaluationRunId: "eval-1",
      expectedRevision: 0,
