import type { CanonicalJsonValue, Sha256Digest } from "@environment-REDACTED/integrations/ports";
...
export interface BraintrustPromotionEvidence {
  ...
  readonly metrics: {
    ...
    readonly REDACTEDLeakageCount: number;
  ...
  readonly state: "running" | "REDACTEDed" | "failed" | "cancelled";
  readonly versionFingerprint: Sha256Digest;
}
...
export type PromotionGateFailure =
  ...
  | "REDACTED_leakage_above_threshold";
...
function evaluatePromotionGate(
  ...
  if (evidence.state !== "REDACTEDed") failures.push("evaluation_not_REDACTEDed");
  if (
    ...
    !Number.isSafeInteger(evidence.metrics.REDACTEDLeakageCount) ||
    evidence.metrics.REDACTEDLeakageCount < 0
  ) {
  ...
  if (evidence.metrics.REDACTEDLeakageCount > policy.maximumSecretLeakageCount) {
    failures.push("REDACTED_leakage_above_threshold");
  }
  return failures;
}
