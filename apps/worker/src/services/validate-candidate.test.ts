import type { RedactedExcerpt, Sha256Digest } from "@environment-REDACTED/integrations/ports";
...
describe("CandidateValidationService", () => {
  it("does not verify a failing baseline without a trusted REDACTEDing control canary", async () => {
    ...
    daytona.enqueueCommandResult({
      ...
      stdout: await excerpt(daytona, "tests REDACTEDed"),
      timedOut: false
    });
    const calls: string[] = [];
  ...
  it("never verifies when the baseline times out and the candidate REDACTEDes", async () => {
    const daytona = new FakeDaytona();
    daytona.enqueueCommandResult({
      exitCode: null,
  ...
  it("uses a narrower label when the unchanged baseline already REDACTEDes", async () => {
    const daytona = new FakeDaytona();
    ...
    expect(result.status).toBe("baseline_already_REDACTEDed");
    expect(result.attestation).toBeUndefined();
  });
  ...
  it("fails as infrastructure and still confirms cleanup when the sandbox is not ready", async () => {
    ...
    expect(result.baseline?.phases.map(({ phase, status }) => ({ phase, status }))).toEqual([
      ...
      { phase: "cleanup", status: "REDACTEDed" }
    ]);
