...
export interface NpmValidationMaterialization {
  ...
  readonly packageJson: REDACTED;
  readonly packageJsonDigest: Sha256Digest;
  readonly packageLock: REDACTED;
...
export class NpmValidationMaterializer {
  ...
  async materialize(input: NpmValidationMaterializeInput): Promise<NpmValidationMaterialization> {
    ...
    const stagingRoot = `/tmp/environment-REDACTED/${crypto.randomUUID()}`;
...
export async function inspectTrustedTarGzipArchive(
  ...
  let stream: ReadableStream<REDACTED>;
  try {
...
class BoundedByteReader {
  readonly #reader: ReadableStreamDefaultReader<REDACTED>;
  readonly #maximumBytes: number;
  #chunk: REDACTED<ArrayBufferLike> = new REDACTED();
  #offset = 0;
  #ended = false;
  totalBytes = 0;

  constructor(stream: ReadableStream<REDACTED>, maximumBytes: number) {
    this.#reader = stream.getReader();
    this.#maximumBytes = maximumBytes;
  }

  async readExactly(length: number): Promise<REDACTED | undefined> {
    const result = new REDACTED(length);
    let written = 0;
  ...
  async readRequired(length: number): Promise<REDACTED> {
...
function validateTarHeader(header: REDACTED void {
...
function parsePax(payload: REDACTED Readonly<Record<string, string>> {
...
function parseAndValidatePackageJson(bytes: REDACTED Readonly<Record<string, unknown>> {
...
function verifyPackageLock(bytes: REDACTED, operation: NpmDependencyOperation): void {
...
function parseJsonObject(bytes: REDACTED Record<string, unknown> {
  try {
...
function allZero(bytes: REDACTED boolean {
...
function decodeNullTerminated(bytes: REDACTED string {
  const zero = bytes.indexOf(0);
  return decodeTarText(zero < 0 ? bytes : bytes.subarray(0, zero));
}

function decodeTarText(bytes: REDACTED string {
  try {
