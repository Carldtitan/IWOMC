...
function passingFacts(): ValidationOutcomeFacts {
  return {
...
describe("classifyValidationOutcome", () => {
  it("emits verified only for a fresh, exactly bound, complete passing target set", () => {
    expect(classifyValidationOutcome(passingFacts())).toEqual({
      attestationDigest,
      status: "verified",
      verified: true,
  ...
  it.each<readonly [TargetExecutionOutcome, Exclude<TargetExecutionOutcome, "passed">, string]>([
  ...
  it("makes cleanup failure terminal even when the preceding target passed", () => {
    const facts = passingFacts();
    expect(
      classifyValidationOutcome({
        ...facts,
  ...
  it("refuses verification for missing contract, target, fresh source, or fresh exact attestation", () => {
    const contractMissing = passingFacts();
    const targetMissing = passingFacts();
    const sourceStale = passingFacts();
    const attestationStale = passingFacts();
    const bindingMismatch = passingFacts();
    const missingAttestation = passingFacts();
