...
export async function deterministicGitHubBranchName(input: {
  ...
  const digest = await sha256Text(
    [
      "environment-reconciler/github-branch/v1",
      input.operationKey,
      input.repositoryId,
  ...
  return `environment-reconciler/${digest.slice("sha256:".length, "sha256:".length + 32)}`;
}
...
export class GitHubRepositoryClient {
  readonly #credentialBroker: GitHubRepositoryCredentialBroker;
  readonly #fetcher: typeof fetch;
  readonly #maxPages: number;
  readonly #pageSize: number;

  constructor(
    credentialBroker: GitHubRepositoryCredentialBroker,
  ...
  async #fetchApi(
    ...
    if (!path.startsWith("/") || token.length === 0) {
      throw new GitHubRepositoryError("invalid_input");
    }
  ...
  async #apiJson(
    ...
    const response = await this.#fetchApi(path, token, init);
  ...
  async #credential(
    ...
    let credential;
    try {
      credential = await this.#credentialBroker.issueRepositoryCredential({
  ...
  async #userJson(path: string, userAccessToken: string): Promise<unknown> {
    if (userAccessToken.length === 0) {
      throw new GitHubRepositoryError("installation_not_authorized");
    }
    const response = await this.#fetchApi(path, userAccessToken);
  ...
  async #listUserInstallations(userAccessToken: string): Promise<
    readonly {
  ...
  > {
    ...
    for (let page = 1; page <= this.#maxPages; page += 1) {
      const body = await this.#userJson(
        `/user/installations?per_page=${this.#pageSize}&page=${page}`,
        userAccessToken
      );
      if (
        !isRecord(body) ||
  ...
  async #listIntersectionRepositories(
    ...
    for (let page = 1; page <= this.#maxPages; page += 1) {
      const body = await this.#userJson(
        `/user/installations/${installationId}/repositories?per_page=${this.#pageSize}&page=${page}`,
        userAccessToken
      );
      if (
        !isRecord(body) ||
  ...
  /**
   * Lists only repositories returned by GitHub's authenticated-user
   * installation endpoints. It never calls the installation-token-wide
   * `/installation/repositories` endpoint.
   */
  async listUserAuthorizedRepositories(
    userAccessToken: string
  ): Promise<readonly GitHubUserInstallation[]> {
    const installations = await this.#listUserInstallations(userAccessToken);
  ...
  async verifyUserRepositoryAuthorization(input: {
    ...
    const installations = await this.#listUserInstallations(input.userAccessToken);
    if (
  ...
  async fetchExactCommitArchive(
    ...
    const token = await this.#credential(request.repository, "contents_read");
    ...
    const initial = await this.#fetchApi(path, token, { method: "GET" });
  ...
  async #getReference(
    ...
    const response = await this.#fetchApi(path, token);
  ...
  async #assertBaseHead(
    ...
    const baseReference = await this.#getReference(repository, repository.defaultBranch, token);
  ...
  async #updateDeterministicBranch(input: {
    ...
    const existing = await this.#getReference(input.repository, input.branchName, input.token);
  ...
  async applyExactChangesAndOpenPullRequest(
    ...
    const contentsToken = await this.#credential(request.repository, "contents_write");
    ...
    const pullRequestToken = await this.#credential(request.repository, "pull_requests_write");
