...
describe("GitHubRepositoryClient repository authorization", () => {
  it("paginates only the authenticated user's installation/repository intersections", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>((input) => {
      ...
      if (url.pathname === "/user/installations/43/repositories") {
    ...
    const installations = await client.listUserAuthorizedRepositories("user-token-secret");
    ...
    expect(requested).toContain("/user/installations?per_page=2&page=2");
    expect(requested).toContain("/user/installations/41/repositories?per_page=2&page=2");
    expect(requested.every((path) => !path.startsWith("/installation/repositories"))).toBe(true);
    expect(issue).not.toHaveBeenCalled();
    for (const [, init] of fetcher.mock.calls) {
      expect(new Headers(init?.headers).get("Authorization")).toBe("Bearer user-token-secret");
    }
  });

  it("rejects spoofed installation and repository IDs from the user intersection", async () => {
  ...
  it("fails closed when pagination exceeds its configured bound", async () => {
    ...
    await expect(client.listUserAuthorizedRepositories("user-token")).rejects.toMatchObject({
...
describe("GitHubRepositoryClient exact archive", () => {
  it("manually follows only the exact codeload commit without forwarding authorization", async () => {
    ...
    expect(calls[0]?.authorization).toBe("Bearer contents_read-secret-token");
  ...
  it("rejects an adversarial redirect without sending a second request or leaking a token", async () => {
    const { broker } = credentialBroker();
    ...
    expect(String(error)).not.toContain("contents_read-secret-token");
...
describe("GitHubRepositoryClient exact pull-request application", () => {
  ...
  it("rejects workflow paths before requesting a repository credential", async () => {
    const fixture = await applyFixture();
