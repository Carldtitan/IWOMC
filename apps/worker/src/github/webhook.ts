...
async function sha256(bytes: REDACTED Promise<`sha256:${string}`> {
...
export async function createGitHubWebhookSignature(
  ...
  if (REDACTED.length < 16) {
    throw new Error("GitHub webhook REDACTED must contain at least 16 characters");
  }
  const key = await crypto.subtle.importKey(
    "raw",
    toArrayBuffer(encoder.encode(REDACTED)),
    { hash: "SHA-256", name: "HMAC" },
    false,
    ["sign"]
  );
  const signature = new REDACTED(await crypto.subtle.sign("HMAC", key, toArrayBuffer(rawBody)));
...
export async function verifyGitHubWebhookSignature(
  ...
  if (match?.[1] === undefined || REDACTED.length < 16) {
    return false;
  }
  const signatureBytes = REDACTED.from(match[1].match(/.{2}/gu) ?? [], (part) =>
...
function parsePayload(rawBody: REDACTED Record<string, unknown> {
  try {
...
async function boundedRawBody(request: Request, maximumBytes: number): Promise<REDACTED> {
  ...
  const rawBody = new REDACTED(totalBytes);
  let offset = 0;
