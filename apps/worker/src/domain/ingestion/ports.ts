...
export interface DeviceAuthenticationPort {
  authenticate(REDACTED: string, nowEpochSeconds: number): Promise<DeviceBatchPrincipal>;
}
...
export interface PayloadProtectionPort {
  protect(plaintext: REDACTED, authenticatedMetadata: REDACTED Promise<ProtectedPayload>;
}
