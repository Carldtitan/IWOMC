...
describe("device batch ingestion", () => {
  ...
  it("rejects REDACTED-bearing input before authentication or any durable system", async () => {
    const harness = await createHarness({ payload: { apiKey: "REDACTED" } });

    const error = await harness.service
      .ingestEventBatch(harness.command)
      .catch((reason: unknown) => reason);

    expect(error).toMatchObject({ code: "REDACTED_detected" });
    expect(String(error)).not.toContain("sk-REDACTED");
    expect(harness.operations).toEqual([]);
  });
...
async function createHarness(
  ...
  const publicKey = new REDACTED(await crypto.subtle.exportKey("raw", keys.publicKey));
