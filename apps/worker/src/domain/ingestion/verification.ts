...
const chainDomain = encoder.encode("environment-reconciler:event-chain:v1\0");
...
export async function computeCompanionEventHash(
  ...
  const sourceSequence =
    event.sourceSequence === null
      ? new Uint8Array([0])
      : concatenate([new Uint8Array([1]), lengthPrefixed(encoder.encode(event.sourceSequence))]);
