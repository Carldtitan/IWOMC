...
import { DefenseInDepthSecretGuard } from "./secret-guard.js";
import type {
  ChainAnchorReceipt,
...
export interface IngestionServiceDependencies {
  ...
  readonly secretGuard?: DefenseInDepthSecretGuard;
}
...
export class IngestionService {
  ...
  constructor(
    ...
    this.#secretGuard = dependencies.secretGuard ?? new DefenseInDepthSecretGuard();
  }
  ...
  async ingestEventBatch(command: IngestEventBatchCommand): Promise<IngestEventBatchResult> {
    ...
    if (!this.#secretGuard.inspect(batch).safe) {
      throw new IngestionError("secret_detected", 400);
    }
  ...
  async #deliverAndReceipt(
    ...
    if (record.state !== "enqueued") {
      ...
      try {
        ...
        // reconciler may safely publish it again and consumers deduplicate by digest.
      }
    }
