...
export interface DefaultOptimalityPolicy {
  ...
  readonly hardGates: readonly [
    ...
    "all_required_targets_REDACTED",
    "native_manager_only",
    "lockfile_consistent",
    "REDACTED_guard_REDACTEDed"
  ];
...
export interface CandidatePolicyFacts {
  ...
  readonly REDACTEDGuardPassed: boolean;
...
export interface CandidatePolicyDecision {
  ...
  readonly resultClass: "verified_eligible" | "reconstruction_REDACTEDed" | "policy_rejected";
}
...
export function evaluateCandidatePolicy(
  ...
  if (!facts.allRequiredTargetsPass) {
    failedHardGates.push("all_required_targets_REDACTED");
  }
  ...
  if (!facts.REDACTEDGuardPassed) {
    failedHardGates.push("REDACTED_guard_REDACTEDed");
  }
  ...
  if (!facts.acceptedBehaviorContract && failedHardGates.length === 1) {
    return {
      ...
      failedHardGates,
      resultClass: "reconstruction_REDACTEDed"
    };
  }
