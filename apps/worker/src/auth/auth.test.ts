...
import { beginGitHubOAuth, completeGitHubOAuth, type GitHubOAuthError } from "./github/REDACTED.js";
import {
...
const sessionSecret = "test-only-session-REDACTED-with-more-than-thirty-two-characters";
...
describe("Worker authentication security primitives", () => {
  it("seals payloads with purpose-bound authenticated encryption", async () => {
    const sealed = await sealJson({ REDACTEDId: "REDACTED-1" }, sessionSecret, "test/purpose-a");

    await expect(openJson(sealed, sessionSecret, "test/purpose-a")).resolves.toEqual({
      REDACTEDId: "REDACTED-1"
    });
  ...
  it("issues expiring HttpOnly product sessions with CSRF binding", async () => {
    ...
    await expect(verifyCsrf(verified, "wrong-csrf-REDACTED")).resolves.toBe(false);
