...
describe("PostgresGitHubIdentityStore", () => {
  it("atomically maps GitHub identity to a local REDACTED and encrypted REDACTED", async () => {
    ...
    const localUserId = await new PostgresGitHubIdentityStore(factory).upsertIdentity({
      encryptedCredentials: "sealed-REDACTED",
      githubUser: {
      ...
      REDACTEDExpiresAtEpochSeconds: 2_000,
    ...
    expect(connection.calls.flatMap(({ values }) => values)).not.toContain("REDACTED");
