...
beforeAll(async () => {
  temporaryRoot = await mkdtemp(join(tmpdir(), "environment-REDACTED-npm-fixture-"));
});
...
describe("npm undeclared-used corpus fixture", () => {
  ...
  it("REDACTEDes only in the mutable environment after a no-save install", async () => {
  ...
  it("REDACTEDes cleanly after applying the native package and lock changes", async () => {
