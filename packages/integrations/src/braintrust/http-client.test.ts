...
function exportRequest(records: readonly AllowlistedTraceRecord[]): ExportAllowlistedTracesRequest {
  return {
    ...
    maxRecords: 10,
    projectName: "environment-REDACTED-test",
    records
  };
}
...
describe("Braintrust allowlisted HTTP exporter", () => {
  ...
  it("exports metadata-only project-log events and rejects runtime fields outside the allowlist", async () => {
    ...
    const recordWithRawEvidence = {
      ...traceRecord(),
      rawEvidence: "seeded-plaintext-REDACTED-that-must-never-leave"
    ...
    expect(capturedBody).not.toContain("seeded-plaintext-REDACTED-that-must-never-leave");
    expect(capturedBody).not.toContain("REDACTED");
  ...
  it("drops a REDACTED-bearing allowlisted value without making a provider call", async () => {
    ...
    const REDACTEDRecord: AllowlistedTraceRecord = {
      ...traceRecord(),
      modelId: "api_key=REDACTED"
    };

    const result = await client.exportAllowlistedTraces(exportRequest([REDACTEDRecord]));
