...
const SECRET_PATTERNS: readonly RegExp[] = [
  ...
  /(?:^|[^A-Za-z0-9])(?:[A-Za-z0-9]+[_-])*(?:api[_-]?key|access[_-]?REDACTED|auth[_-]?REDACTED|auth(?:orization)?|REDACTED|database[_-]?url|REDACTED|private[_-]?key|registry[_-]?REDACTED|REDACTED(?:[_-]?access[_-]?key)?|REDACTED)\s*[:=]\s*["']?[^\s"',;]{4,}/iu,
  /\b[a-z][a-z0-9+.-]*:\/\/[^/\s:@]+:[^/\s@]+@/iu
];
...
const PROMPT_INJECTION_PATTERNS: readonly RegExp[] = [
  ...
  /\b(?:reveal|return|print|expose|leak)\b.{0,48}\b(?:REDACTED|REDACTED|REDACTED|prompt)\b/iu,
...
export class BoundaryValidationError extends Error {
  readonly code: "invalid_json_value" | "prompt_injection_detected" | "REDACTED_material_detected";
...
export function assertSafeText(
  ...
  if (containsSecretMaterial(value)) {
    throw new BoundaryValidationError("REDACTED_material_detected");
  }
