...
import { parseServerEnvironment } from "@environment-REDACTED/contracts/env";
...
async function run(): Promise<void> {
  ...
  const REDACTEDAccessKey = environment.R2_S3_SECRET_ACCESS_KEY;
  const bucket = environment.R2_BUCKET_NAME;

  if (
    endpoint === undefined ||
    accessKeyId === undefined ||
    REDACTEDAccessKey === undefined ||
    bucket === undefined
  ) {
  ...
  const client = new S3Client({
    REDACTEDs: { accessKeyId, REDACTEDAccessKey },
    endpoint,
    region: "auto"
  });
  const objectKey = `_environment-REDACTED-smoke/${crypto.randomUUID()}.txt`;
  const expected = new TextEncoder().encode(
    `environment-REDACTED-r2-smoke:${crypto.randomUUID()}`
  );
  ...
  try {
    ...
    await client.send(
      new PutObjectCommand({
        ...
        Key: objectKey,
        Metadata: {
          purpose: "environment-REDACTED-smoke"
        }
      })
    );
