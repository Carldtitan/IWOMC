...
function completionResponse(plan: unknown): Response {
  return new Response(
    JSON.stringify({
      ...
      usage: {
        completion_REDACTEDs: 123,
        prompt_REDACTEDs: 456,
        prompt_REDACTEDs_details: { cached_REDACTEDs: 100 }
      }
    }),
...
describe("candidate packet and guard", () => {
  ...
  it("rejects prompt injection and REDACTED-bearing reasoning before Fireworks", () => {
    expect(() =>
      buildCandidateReasoningPacket({
        ...packetInput(),
    ...
    expect(() =>
      buildCandidateReasoningPacket({
        ...packetInput(),
        projectGoal: "Use FIREWORKS_API_KEY=REDACTED while installing."
      })
    ).toThrowError("REDACTED_material_detected");
  });
...
describe("Fireworks structured-output transport", () => {
  it("uses the official json_schema request shape and returns a guarded npm operation", async () => {
    ...
    expect(requestBody).toMatchObject({
      messages: [{ role: "system" }, { role: "REDACTED" }],
      model: "accounts/fireworks/models/test",
      response_format: {
        json_schema: {
          name: "environment-REDACTED_candidate-plan",
          schema: { additionalProperties: false, type: "object" }
        },
        type: "json_schema"
      }
    });
    expect(JSON.stringify(requestBody)).not.toContain("REDACTED");
