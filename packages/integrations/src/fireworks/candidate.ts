...
export const CANDIDATE_PLAN_SCHEMA_ID = "environment-reconciler.candidate-plan";
...
export interface PriorValidationSummary {
  ...
  readonly summaryClass:
    "baseline-failed" | "baseline-passed" | "candidate-failed" | "candidate-passed" | "not-run";
}
...
export interface ExpectedValidationImpact {
  readonly targetId: string;
  readonly expectedOutcome: "pass" | "unchanged";
  ...
  properties: {
    ...
    expectedValidationImpact: {
      items: {
        additionalProperties: false,
        properties: {
          expectedOutcome: { enum: ["pass", "unchanged"], type: "string" },
...
function parseExpectedValidationImpact(value: unknown): ExpectedValidationImpact {
  ...
  if (expectedOutcome !== "pass" && expectedOutcome !== "unchanged") {
    invalidStructuredOutput();
  }
  return {
    expectedOutcome,
