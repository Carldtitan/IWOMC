...
function parseProviderResponse(value: unknown): ParsedFireworksResponse {
  ...
  const promptDetails = isRecord(usageRecord.prompt_tokens_details)
  ...
  const usage: FireworksTokenUsage = {
    ...
    inputTokens: nonNegativeInteger(usageRecord.prompt_tokens),
    outputTokens: nonNegativeInteger(usageRecord.completion_tokens)
  };
  return {
    finishReason,
    output,
...
export class FireworksHttpClient implements FireworksPort {
  ...
  async generateStructured(
    ...
    const body = JSON.stringify({
      max_tokens: request.sampling.maxOutputTokens,
      messages: [
