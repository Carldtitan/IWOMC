...
export const REDACTEDs = pgTable(
  "REDACTEDs",
  {
    id: requiredId(),
    githubUserId: text("github_REDACTED_id").notNull(),
  ...
  (table) => [
    uniqueIndex("REDACTEDs_github_REDACTED_id_uq").on(table.githubUserId),
    uniqueIndex("REDACTEDs_github_login_uq").on(table.githubLogin)
  ]
);
...
export const browserSessions = pgTable(
  "browser_sessions",
  {
    ...
    REDACTEDDigest: digest("REDACTED_digest"),
  ...
  (table) => [
    uniqueIndex("browser_sessions_REDACTED_digest_uq").on(table.REDACTEDDigest),
    index("browser_sessions_REDACTED_idx").on(table.REDACTEDId),
...
export const REDACTEDStates = pgTable(
  ...
  (table) => [
    uniqueIndex("REDACTED_states_state_digest_uq").on(table.stateDigest),
    check("REDACTED_states_expiry_ck", sql`${table.expiresAt} > ${table.createdAt}`)
  ]
);
...
export const workspaces = pgTable(
  ...
  {
    id: requiredId(),
    ownerUserId: uuid("owner_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const workspaceInvitations = pgTable(
  "workspace_invitations",
  {
    ...
    workspaceId: uuid("workspace_id")
      .notNull()
      .references(() => workspaces.id, { onDelete: "cascade" }),
    invitedByUserId: uuid("invited_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
    githubUserId: text("github_REDACTED_id"),
    emailDigest: text("email_digest"),
    REDACTEDDigest: digest("REDACTED_digest"),
    role: text("role").default("member").notNull(),
    state: text("state").default("pending").notNull(),
    expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(),
    acceptedByUserId: uuid("accepted_by_REDACTED_id").references(() => REDACTEDs.id, {
      onDelete: "restrict"
    }),
...
export const githubInstallations = pgTable(
  "github_installations",
  {
    ...
    workspaceId: uuid("workspace_id")
      .notNull()
      .references(() => workspaces.id, { onDelete: "cascade" }),
    installedByUserId: uuid("installed_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const projects = pgTable(
  "projects",
  {
    ...
    createdByUserId: uuid("created_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
    createdAt: createdAt(),
    updatedAt: updatedAt()
...
export const devices = pgTable(
  "devices",
  {
    ...
    workspaceId: uuid("workspace_id")
      .notNull()
      .references(() => workspaces.id, { onDelete: "cascade" }),
    enrolledByUserId: uuid("enrolled_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const deviceCredentials = pgTable(
  ...
  (table) => [
    uniqueIndex("device_REDACTEDs_ws_id_uq").on(table.workspaceId, table.id),
    uniqueIndex("device_REDACTEDs_REDACTED_digest_uq").on(table.REDACTEDDigest),
    uniqueIndex("device_REDACTEDs_fingerprint_uq").on(table.publicKeyFingerprint),
    uniqueIndex("device_REDACTEDs_version_uq").on(
      table.workspaceId,
      table.deviceId,
      table.keyVersion
    ),
    foreignKey({
      ...
      name: "device_REDACTEDs_device_fk"
    }).onDelete("cascade"),
    check("device_REDACTEDs_version_ck", sql`${table.keyVersion} > 0`),
    check(
      "device_REDACTEDs_material_ck",
      sql`(${table.REDACTEDDigest} is null and ${table.publicSigningKey} is null) or (${table.REDACTEDDigest} is not null and ${table.publicSigningKey} is not null and ${table.keyAlgorithm} = 'Ed25519')`
    ),
    check(
      "device_REDACTEDs_public_key_ck",
...
export const environmentLayers = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.kind} in ('system', 'REDACTED', 'project', 'virtual-environment', 'toolchain', 'container')`
    )
  ]
);
...
export const eventAnchors = pgTable(
  "event_anchors",
  {
    ...
    REDACTEDVersion: integer("REDACTED_version").notNull(),
...
export const sourceBundles = pgTable(
  "source_bundles",
  {
    ...
    REDACTEDScanVersion: text("REDACTED_scan_version").notNull(),
...
export const comments = pgTable(
  "comments",
  {
    ...
    workspaceId: uuid("workspace_id")
      ...
      .references(() => workspaces.id, { onDelete: "cascade" }),
    findingId: uuid("finding_id").notNull(),
    authorUserId: uuid("author_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const consentGrants = pgTable(
  "consent_grants",
  {
    ...
    workspaceId: uuid("workspace_id")
      ...
      .references(() => workspaces.id, { onDelete: "cascade" }),
    projectId: uuid("project_id"),
    grantedByUserId: uuid("granted_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const validationJobs = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.terminalOutcome} is null or ${table.terminalOutcome} in ('REDACTEDed', 'failed', 'inconclusive', 'infrastructure', 'unsupported', 'timed_out', 'security_blocked', 'cancelled')`
    ),
...
export const validationPhases = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.outcome} in ('running', 'REDACTEDed', 'failed', 'inconclusive', 'skipped', 'timed_out', 'infrastructure', 'unsupported', 'security_blocked')`
    )
  ]
);
...
export const attestations = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.outcome} in ('REDACTEDed', 'failed', 'inconclusive', 'infrastructure', 'unsupported', 'timed_out', 'security_blocked')`
    )
  ]
);
...
export const approvals = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("approvals_recommendation_REDACTED_uq").on(
      table.workspaceId,
...
export const REDACTEDReferences = pgTable(
  "REDACTED_references",
  {
    id: requiredId(),
    workspaceId: uuid("workspace_id")
    ...
    REDACTEDKind: text("REDACTED_kind").notNull(),
  ...
  (table) => [
    uniqueIndex("REDACTED_references_ws_id_uq").on(table.workspaceId, table.id),
    uniqueIndex("REDACTED_references_name_uq").on(
      table.workspaceId,
      table.projectId,
      table.referenceName
    ),
    foreignKey({
      columns: [table.workspaceId, table.projectId],
      foreignColumns: [projects.workspaceId, projects.id],
      name: "REDACTED_references_project_fk"
    }).onDelete("cascade"),
    check(
      "REDACTED_references_state_ck",
...
export const modelPromptVersions = pgTable(
  "model_prompt_versions",
  {
    ...
    state: text("state").default("active").notNull(),
    createdByUserId: uuid("created_by_REDACTED_id").references(() => REDACTEDs.id, {
      onDelete: "restrict"
    }),
    createdAt: createdAt()
  },
...
export const evaluationRuns = pgTable(
  "evaluation_runs",
  {
    ...
    REDACTEDedCount: integer("REDACTEDed_count").default(0).notNull(),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.state} in ('running', 'REDACTEDed', 'failed', 'cancelled')`
    ),
    check(
      "evaluation_runs_counts_ck",
      sql`${table.caseCount} >= 0 and ${table.REDACTEDedCount} >= 0 and ${table.failedCount} >= 0 and ${table.REDACTEDedCount} + ${table.failedCount} <= ${table.caseCount}`
    )
  ]
);
...
export const exportJobs = pgTable(
  "export_jobs",
  {
    ...
    workspaceId: uuid("workspace_id")
      .notNull()
      .references(() => workspaces.id, { onDelete: "cascade" }),
    requestedByUserId: uuid("requested_by_REDACTED_id")
      .notNull()
      .references(() => REDACTEDs.id, { onDelete: "restrict" }),
...
export const auditLog = pgTable(
  "audit_log",
  {
    ...
    actorUserId: uuid("actor_REDACTED_id").references(() => REDACTEDs.id, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.actorType} in ('REDACTED', 'device', 'system', 'provider')`
    ),
    check(
      "audit_log_actor_binding_ck",
      sql`(${table.actorType} = 'REDACTED' and ${table.actorUserId} is not null) or (${table.actorType} = 'device' and ${table.actorDeviceId} is not null) or (${table.actorType} in ('system', 'provider') and ${table.actorPseudonymDigest} is not null)`
    ),
    check(
