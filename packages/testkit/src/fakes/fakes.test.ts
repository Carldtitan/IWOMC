...
describe("deterministic external-service fakes", () => {
  ...
  it("exports only allowlisted Braintrust records within explicit bounds", async () => {
    ...
    const request = {
      ...
      projectName: "environment-reconciler",
      records: [trace],
      maxRecords: 10,
    ...
    expect(braintrust.records("environment-reconciler")).toHaveLength(1);

    const recordWithUnexpectedField = { ...trace, sourceCode: "secret" };
    await expect(
  ...
  it("models exact GitHub source, commit, compare-and-swap branch, and PR operations", async () => {
    ...
    await github.seedRepository({
      ...
      commits: [
        {
          commitSha: baseSha,
          files: [{ path: "package.json", content: Uint8Array.from([123, 125]) }],
          zipArchive: Uint8Array.from([80, 75, 3, 4])
        }
      ],
    ...
    const lockContent = Uint8Array.from([108, 111, 99, 107]);
    ...
    expect(github.snapshot(repository)).toEqual({
      branches: {
        main: baseSha,
        "reconciler/run-1": commit.commitSha
      },
      commitCount: 2,
      pullRequestCount: 1
    });
  ...
  it("reconciles Daytona lifecycle by operation key and executes only scripted commands", async () => {
    ...
    const stdout = await excerpt(scenario, "tests passed");
