...
interface WebCryptoRuntime {
  readonly subtle: {
    digest(algorithm: "SHA-256", content: Uint8Array): Promise<ArrayBuffer>;
  };
}
...
export class CanonicalSha256Hasher implements ContentHasher {
  async hashBytes(content: Uint8Array): Promise<Sha256Digest> {
    const stableCopy = Uint8Array.from(content);
