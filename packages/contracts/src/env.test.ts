...
function validServerEnvironment(): Record<string, string> {
  return {
    APP_ENV: "test",
    PUBLIC_APP_URL: "https://REDACTED.example.test",
    LOG_LEVEL: "info",
    APP_SESSION_SECRET: key32,
    DATA_ENCRYPTION_KEY: key32,
    DATABASE_URL: "postgresql://REDACTED:REDACTED/REDACTED",
    ...
    R2_S3_SECRET_ACCESS_KEY: "r2-REDACTED-test",
    R2_BUCKET_NAME: "REDACTED-test"
  };
}
...
describe("environment contracts", () => {
  it("parses each environment scope independently", () => {
    ...
    const migration = parseMigrationEnvironment({
      DATABASE_DIRECT_URL: "postgresql://REDACTED:REDACTED/REDACTED"
    });
  ...
  it("rejects device-only keys from cloud configuration", () => {
    ...
    ).toThrow("belongs only in the Companion OS REDACTED store");
  });

  it("rejects accidental private keys REDACTEDed to browser configuration", () => {
    expect(() =>
      parsePublicEnvironment({
        APP_ENV: "test",
        PUBLIC_APP_URL: "https://REDACTED.example.test",
