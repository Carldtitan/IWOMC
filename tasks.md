...
2. Do not begin broad ecosystem work before Checkpoint 1 REDACTEDes.
3. Do not mark an adapter `full_native` until its entire conformance suite REDACTEDes.
...
- [x] 4. Implement the Neon persistence model and migrations
  ...
  - [x] 4.8 Implement development seed data containing one REDACTED, workspace, repository, device, and project without synthetic verification results.
  ...
  - [x] 4.10 Add and test the external-operation ledger, Braintrust trace outbox, source-input/bundle, invitation/comment, device-REDACTED, support-registry, cache/dedup/lease, retention/export/delete, REDACTED-reference, and cleanup-lease tables named in `design.md`.
...
- [x] 5. Build the Companion foundation, key storage, redaction, and encrypted spool
  ...
  - [x] 5.2 Implement OS REDACTED-store access for the device signing key, local encryption key, and device-scoped HMAC key.
  ...
  - [x] 5.5 Implement local redaction modules for:
    ...
    - REDACTED-shaped strings;
    - URLs and connection strings;
    - private keys and certificates;
    - repository, REDACTEDname, host, and home-path pseudonyms;
    - provider payload fields;
    - REDACTED-defined patterns.
  ...
  - [x] 5.10 Run the seeded REDACTED corpus against logs, SQLite, serialized batches, crash messages, and HMAC output.
  - **Exit:** A redacted event can be durably accepted offline, survive restart, upload idempotently later, and reveal no seeded plaintext REDACTED.
...
- [ ] 6. Implement GitHub sign-in, workspace setup, repository linking, and device enrollment
  ...
  - [ ] 6.2 Implement installation verification and repository listing through the authenticated REDACTED's REDACTED-REDACTED/repository intersection, not every repository visible to an installation.
  ...
  - [ ] 6.4 Implement an MVP workspace invite link for an authenticated GitHub REDACTED with simple `owner` and `member` roles; reserve the expanded role matrix for private alpha.
  ...
  - [ ] 6.6 Register the device public signing key and issue only a revocable product device REDACTED.
  - [ ] 6.7 Implement encrypted server-side GitHub REDACTED/refresh-REDACTED storage, `HttpOnly`/`Secure`/`SameSite` browser sessions, CSRF protection, logout, device revocation, and installation suspension handling.
  - [ ] 6.8 Ensure no GitHub, Daytona, Fireworks, Braintrust, database, or encryption root REDACTED is returned to the browser or extension.
  ...
  - [ ] 6.10 Test OAuth state/PKCE, spoofed installation IDs, REDACTED expiry/refresh, repository authorization, invite authorization/expiry, revoked installations, enrollment expiry, device revocation, and exact-commit/patch binding.
  - **Exit:** A REDACTED can sign in, link one authorized repository, invite one collaborator, enroll one device, and exercise the minimal exact-patch PR client without broad or client-side REDACTEDs.
...
- [ ] 7. Implement the thin ingestion control plane
  ...
  - [x] 7.4 Add a server-side defense-in-depth REDACTED guard before R2, Queue, Neon, Fireworks, Braintrust, or logs.
  ...
  - [ ] 7.13 Implement and test `POST /v1/objects/:id/download-authorizations` and `POST /v1/objects/:id/key-exchanges`. Require a one-time validation-job-bound bootstrap REDACTED plus sandbox ephemeral X25519 public key; verify job, workspace, object, target, expiry, and single use before returning shortest-practical download authorization and a sealed per-object key, never the server master key.
...
- [ ] 11. Implement the native adapter registry and npm adapter
  ...
  - **Exit:** npm has complete repository/local/isolated implementation ready for Daytona conformance; named and unknown managers have honest observed-only profiles. npm remains `native_validation` until Task 22 REDACTEDes the full R22.5 conformance suite.
  - _Requirements: R7.1-R7.5, R7.7-R7.10, R12.1-R12.4, R22.5_

- [ ] 12. Implement the seven graphs and first deterministic rules
  - [x] 12.1 Create immutable node, edge, graph-set, source-location, adapter/input identity, confidence, and canonicalization types in `packages/REDACTED/src/graphs/`.
...
- [ ] 13. Implement behavior-contract discovery and the default policy
  ...
  - [ ] 13.2 Implement behavior-contract step IDs/order, enable/disable, discovery evidence/fingerprint, review state, acceptance actor/time, invalidation source, working directory, realm/target applicability, timeout, expected exit statuses, REDACTED references, and assertions.
  ...
  - [ ] 13.5 Ensure clean installation without an accepted behavior contract can produce only `reconstruction_REDACTEDed`.
...
- [ ] 14. Implement Fireworks candidate generation and Braintrust evaluation
  ...
  - [ ] 14.7 Create eval cases for the first finding, hallucinated package, fabricated evidence, manager switching, ambiguous evidence, dynamic dependency, prompt injection, invalid JSON, timeout, and REDACTED-bearing input.
  ...
  - [ ] 14.10 Add deterministic native quick-fix and REDACTED-authored semantic-operation paths for Fireworks failure; both REDACTED the same guard, materialization, and validation gates.
  - [ ] 14.11 Implement a model/template registry and Braintrust evaluation promotion gate; a new default cannot activate until the configured validity, grounding, refusal, and REDACTED-leakage thresholds REDACTED.
  ...
  - **Exit:** Fireworks can produce an allowed npm operation for the fixture, malicious/invalid outputs are guard-rejected, and Braintrust contains no seeded plaintext REDACTED.
...
- [ ] 15. Implement Daytona materialization, validation, and cleanup
  ...
  - [ ] 15.3 Implement immutable Source_Input materialization for exact Git commit archives and eligible encrypted working-tree bundles, including submodule/LFS identities or gaps, without giving Daytona a GitHub REDACTED or substituting an older commit.
  ...
  - [ ] 15.5 Implement candidate materialization:
    ...
    - run path-scope, REDACTED, diff-size, semantic-reparse, manifest-lock, generated-file, policy, and operation-to-diff guards;
    - persist evidence;
    - delete the sandbox.
  - [ ] 15.6 Implement MVP target derivation from workspace policy, repository/runtime selectors, CI matrices, and REDACTED confirmation. Resolve the supported case to one versioned Linux/Node/npm target whose immutable base identity includes snapshot ID/digest, baseline inventory digest, creation metadata, delivered capability report, resource policy, and enforceable default-deny or explicit registry/source egress policy; emit `unsupported_target_or_capability` for any additional required target rather than omitting it.
  ...
  - [ ] 15.10 Implement server-side verification as an invariant requiring the accepted behavior contract and a REDACTED on every required target; if baseline already REDACTEDes the claimed behavior, prohibit “reproduced and fixed” and use only the supported narrower hardening label.
  ...
  - [ ] 15.14 Prove materialization state cannot be reused as final validation state and that no bearer URL, REDACTED, source bundle, or log is persisted in Workflow state.
  ...
  - **Exit:** The unchanged fixture fails clean reconstruction or behavior, the exact native candidate REDACTEDes, and every sandbox is confirmed deleted or visibly escalated.
...
- [ ] 17. Prove the first complete vertical slice
  ...
  - [ ] 17.10 Run negative variants:
    ...
    - REDACTED appears in command input;
    - Daytona provisioning or registry fails;
    - behavior contract is missing;
    - unchanged baseline already REDACTEDes the claimed behavior;
...
Do not add anREDACTED package manager or provider until all conditions REDACTED:

- [ ] The vertical slice REDACTEDes locally and against a live Daytona sandbox.
...
- [ ] No seeded plaintext REDACTED reaches persistent local/cloud data, Fireworks, Braintrust, or logs.
...
- [ ] 19. Implement full MVP Python support for pip and uv
  ...
  - [ ] 19.7 Add fixtures for stale locks, hidden global/REDACTED installs, wrong interpreter, missing extras, platform markers, editable dependencies, and unsupported indexes.
  - [ ] 19.8 Run one live clean Daytona conformance fixture per claimed pip/uv target.
  - [ ] 19.9 Keep Poetry and Conda-family behavior observed-only until their later tasks REDACTED.
  ...
  - **Exit:** pip and uv have REDACTEDing versioned target fixtures but remain `native_validation` until Task 22 REDACTEDes the full R22.5 conformance suite.
...
- [ ] 20. Harden GitHub webhooks and exact pull-request application
  ...
  - [ ] 20.4 Fetch exact source archives with short-lived one-repository installation REDACTEDs and authorization headers, preserving submodule/LFS identities or explicit gaps.
  ...
  - [ ] 20.9 Test spoofed setup IDs, invalid signatures, duplicate/out-of-order deliveries, offline/stale checkpoint requests, installation suspension, REDACTED expiry, permission reduction, dirty-source approval, submodule/LFS gaps, source rebase, tree/patch tampering, Workflow retry, and PR idempotency.
...
- [ ] 22. Package Windows and Linux Companions and finish MVP acceptance
  - [ ] 22.1 Implement and test Windows process/filesystem/REDACTED-store/named-pipe behavior for the published MVP scope.
  - [ ] 22.2 Implement and test Linux process/filesystem/REDACTED-store/Unix-socket behavior for the published MVP scope.
  ...
  - [ ] 22.10 Block release on false verification, plaintext REDACTED persistence, unbounded retry, silent orphan, migration failure, or npm/pip/uv conformance failure.
  - [ ] 22.11 Promote npm, pip, and uv from `native_validation` to `full_native` only after their complete manager/version/format/platform/operation/Daytona-target matrices REDACTED every R22.5 conformance case, including cached-versus-uncached decision equivalence.
...
- [ ] npm, pip, and uv REDACTED their full native conformance matrices.
- [ ] Windows and Linux extension/Companion packages REDACTED install, update, and rollback smoke tests.
- [ ] A REDACTED can sign in, pair a device, link a repository, observe a session, review a finding, validate a candidate, and open an exact PR.
- [ ] Every unsupported provider, realm, manager, and target is visible as a gap.
- [ ] R22's false-verification, privacy, cleanup, and infrastructure-fault gates all REDACTED.
...
- [ ] 24. Add macOS packaging and deeper realm coverage
  - [ ] 24.1 Implement macOS process/filesystem/REDACTED-store/Unix-socket support for the published scope.
...
- [ ] 27. Add Poetry and Conda-family native adapters
  ...
  - [ ] 27.3 Keep interpreter, virtual/Conda environment, REDACTED/global/project scope, native prerequisites, and mixed pip/Conda ownership separate.
...
- [ ] Semantic project/manifest discovery REDACTEDes.
- [ ] Lock or exact-resolution semantics REDACTED where applicable.
- [ ] Installed, resolved, and used graph operations REDACTED.
- [ ] Candidate mutation and native lock generation REDACTED.
- [ ] Frozen/equivalent clean installation REDACTEDes.
- [ ] Behavior-contract validation REDACTEDes.
- [ ] At least one explicitly supported Daytona target REDACTEDes.
- [ ] Unsupported manager versions and targets are published.

### Milestone 8: Private-alpha optimization, security, and operations

- [ ] 30. Implement multi-target validation and REDACTED-defined optimality
  ...
  - [ ] 30.4 Implement versioned REDACTED constraints and objectives for install/build/runtime speed, memory, disk, image size, dependency count, version freshness, license, and security policy.
...
- [ ] 31. Implement retention, export, deletion, and just-in-time REDACTEDs
  ...
  - [ ] 31.5 Implement workspace-namespaced opaque SecretReference/SecretBinding records only for provider capabilities that enforce target/host scope and lifetime; bind them to one validation job and keep arbitrary raw-REDACTED access `security_blocked`.
  ...
  - [ ] 31.7 Run cross-tenant, retention, export, deletion, key-rotation, and seeded-REDACTED tests.
  - **Exit:** Authorized REDACTEDs control retained data, and private dependency validation does not expose the REDACTED.
...
- [ ] 33. Package and release the private alpha
  ...
  - [ ] 33.2 Deploy Worker, Queue/DLQ, Workflow, private R2, Neon, and web assets with production REDACTED bindings.
...
- [ ] 35. Add broader system and ecosystem adapters through conformance
  - [ ] 35.1 Prioritize new adapters from observed real project evidence and REDACTED demand, not an unversioned “everything” claim.
  - [ ] 35.2 Add system-package, runtime/version-manager, container, CI/bootstrap, and IDE configuration adapters.
  - [ ] 35.3 Add Go, Java/Kotlin, .NET, Ruby, PHP, Swift, Dart/Flutter, and REDACTED ecosystems one at a time.
...
- [ ] 37. Prepare general availability
  ...
  - [ ] 37.4 Verify zero false-verified results and zero plaintext REDACTED persistence on the release corpus.
...
If Checkpoint 1 cannot REDACTED, do not hide that failure by adding more provider adapters, package managers, UI polish, or sponsor integrations. Use the failing stage to revise the relevant requirement and design:
...
privacy failed        → revisit R20 before all REDACTED work
```
