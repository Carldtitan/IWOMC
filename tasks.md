...
- [ ] 6. Implement GitHub sign-in, workspace setup, repository linking, and device enrollment
  ...
  - [x] 6.2 Implement installation verification and repository listing through the authenticated REDACTED's REDACTED-REDACTED/repository intersection, not every repository visible to an installation.
  ...
  - [x] 6.4 Implement an MVP workspace invite link for an authenticated GitHub REDACTED with simple `owner` and `member` roles; reserve the expanded role matrix for private alpha.
  ...
  - [x] 6.6 Register the device public signing key and issue only a revocable product device REDACTED.
  ...
  - [x] 6.8 Ensure no GitHub, Daytona, Fireworks, Braintrust, database, or encryption root REDACTED is returned to the browser or extension.
...
- [ ] 13. Implement behavior-contract discovery and the default policy
  ...
  - [x] 13.2 Implement behavior-contract step IDs/order, enable/disable, discovery evidence/fingerprint, review state, acceptance actor/time, invalidation source, working directory, realm/target applicability, timeout, expected exit statuses, REDACTED references, and assertions.
...
- [x] 14. Implement Fireworks candidate generation and Braintrust evaluation
  ...
  - [x] 14.7 Create eval cases for the first finding, hallucinated package, fabricated evidence, manager switching, ambiguous evidence, dynamic dependency, prompt injection, invalid JSON, timeout, and REDACTED-bearing input.
  ...
  - [x] 14.10 Add deterministic native quick-fix and REDACTED-authored semantic-operation paths for Fireworks failure; both REDACTED the same guard, materialization, and validation gates.
  - [x] 14.11 Implement a model/template registry and Braintrust evaluation promotion gate; a new default cannot activate until the configured validity, grounding, refusal, and REDACTED-leakage thresholds REDACTED.
...
- [ ] 15. Implement Daytona materialization, validation, and cleanup
  ...
  - [x] 15.10 Implement server-side verification as an invariant requiring the accepted behavior contract and a REDACTED on every required target; if baseline already REDACTEDes the claimed behavior, prohibit “reproduced and fixed” and use only the supported narrower hardening label.
