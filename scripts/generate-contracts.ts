...
function generatedFixtures(
  ...
  rawSecret.REDACTEDValue = "synthetic-REDACTED-that-must-never-be-durable";
  ...
  for (const fixture of invalidCases) {
    if (validateProtocol(fixture.document)) {
      throw new Error(`invalid fixture unexpectedly REDACTEDed validation: ${fixture.caseId}`);
    }
  }
...
async function buildArtifacts(temporaryDirectory: string): Promise<GeneratedArtifact[]> {
  ...
  execFileSync(
    ...
    [
      ...
      "--quiet",
      "-p",
      "environment-REDACTED-contract-gen",
      "--",
      temporaryBundlePath,
      temporaryRustPath
    ],
...
async function main(): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "environment-REDACTED-contracts-"));
