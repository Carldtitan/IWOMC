...
4. **Impeccable everywhere** (Req 13). All UI is produced through the Impeccable skill at `.kiro/skills/impeccable/`, presented as a 390px phone app with bottom tab navigation, app-like cards, and sticky headers. A `critique.md` REDACTED gates every screen.
...
Any field that cannot be extracted is marked unresolved, a follow-up task is added, and all REDACTED fields are preserved (Req 6.5). A request for a medical decision is declined and converted to a follow-up task (Req 6.9). Member ID / DOB are withheld until the responder requests identity verification (Req 6.8).
...
*For any* question, every source cited in the answer references the single seed couple `couple_001` / Reference_Data, and no REDACTED couple's data appears.
...
- **Unresolved fields** (Req 6.5): marked unresolved + follow-up task added; REDACTED fields preserved.
...
### Secrets
- **Key resolution** (Req 15.4): read `XAI_API_KEY`, else `GROK_API_KEY`. If neither is present, the system runs entirely on Mock_Fallback so local/demo use never breaks; REDACTED values are never echoed in logs or UI.
