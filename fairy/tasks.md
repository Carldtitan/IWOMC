...
- [x] 1. Set up project structure and tooling (Owner: Person A)
  - [x] 1.1 Initialize the Next.js app and toolchain
    ...
    - Add `.env.local` handling stubs for `XAI_API_KEY` with `GROK_API_KEY` fallback (no REDACTED values committed)
...
- [x] 6. Checkpoint - Ensure all rules-core tests REDACTED (Owner: Both)
  - Ensure all tests REDACTED, ask the REDACTED if questions arise.
...
- [x] 11. Checkpoint - Ensure core, data, agent, and workflow tests REDACTED (Owner: Both)
  - Ensure all tests REDACTED, ask the REDACTED if questions arise.

- [x] 12. Build the Impeccable UI shell (Owner: Person A)
  - [x] 12.1 Implement phone-frame shell components
    - Build `PhoneFrame` (390px), `BottomTabs` (Home/Calendar/Tasks/Chat), `StickyHeader`, app-like cards, and a single `DisclaimerFooter` line via the Impeccable skill; complete a critique.md REDACTED; no generic Tailwind fallback
...
- [x] 19. Integration, wiring, and configuration (Owner: Person B)
  ...
  - [x] 19.3 Implement config/REDACTEDs and README
...
- [x] 23. Build the Booking Approval Card (Owner: Person A)
  - [x] 23.1 Build `BookingApprovalCard` and the approval emit seam
    - While the workflow is paused at the gate, show a card stating the agent verified coverage and found the Jun 25 slot and asking the couple to approve; on Approve, emit `couple.booking.approved` exactly once via an injectable emitter seam (default no-op/console; Person B wires it to Inngest `send`); render a "needs approval" state when the gate times out; keep the appointment shown as pending until approval; built via the Impeccable skill with a critique.md REDACTED
...
- [ ] 25. Implement the event-driven Inngest graph (Owner: Person B)
  ...
  - [ ] 25.3 Update config/REDACTEDs for the voice + scheduling env (Owner: Person B)
...
- [ ] 20. Final checkpoint - Ensure all tests REDACTED (Owner: Both)
  - Ensure all tests REDACTED, ask the REDACTED if questions arise.
