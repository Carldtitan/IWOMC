...
This design implements the MVP contract in [requirements.md](requirements.md) first. The adapter, provider, realm, and target registries allow later expansion, but the product never claims support beyond the exact capabilities that have passed conformance.
...
8. **Privacy begins locally.** Redaction, secret blocking, scoped inventory, and durable buffering happen before cloud ingestion.  
...
12. **No silent fallback.** A failed observer, model, trace exporter, sandbox, or integration changes capability or workflow status; it never fabricates evidence or a pass.
...
| Local state | SQLite WAL + OS credential store | Encrypted offline spool, sequence cursors, snapshot indexes, device signing/fingerprint keys |
...
|---|---|---|
| Local application | Extension, companion, OS credential store, local SQLite | Secrets and observational raw state do not cross this boundary; private alpha may add a separately governed opt-in class |
...
| Observability | Braintrust | Receives pseudonymous metadata, trace structure, scores, latency, and fingerprints; no raw secret or private reasoning |
| Untrusted execution | Every Daytona sandbox and checked-out repository | Short lifetime, least privilege, egress policy, no product root credentials, mandatory cleanup |
| Repository provider | GitHub | Short-lived, installation-scoped or user-scoped tokens; no GitHub credential reaches an extension bundle or sandbox |
...
The hook receiver never waits for cloud reconciliation. After a material action stabilizes, the Companion asynchronously creates the targeted checkpoint/reconciliation trigger; session end, relevant PR update, and **Scan now** create the other required triggers.
...
flowchart TD
    ...
    Patch --> PatchGuard[Post-materialization semantic + secret + scope guards]
    ...
    Contract -- yes --> Complete{Every required target passed?}
...
The extension is a thin control and notification surface. It is not the evidence source of truth and does not contain sponsor credentials.
...
| **Validate candidate** | A candidate passed static guards | Starts or resumes the Daytona workflow |
...
SQLite uses WAL mode. Sensitive payload fields are encrypted with AES-256-GCM under a versioned data key protected by the OS credential store. A separate OS-held HMAC key produces device-scoped equality fingerprints. The local database never stores the plaintext key.
...
The reconciler package is pure: no network calls, current time, random values, or mutable global state. Identical graph/policy/rule inputs produce byte-equivalent normalized finding output.
...
The MVP has no raw secret entry path and returns `security_blocked` for private installs/tests. Private alpha may namespace opaque references per workspace and bind only host-restricted or equivalently enforceable credentials to one job. Arbitrary raw secret access by untrusted repository code remains blocked. Product code never resolves a value into Workflow state, command arguments, cache keys, Fireworks, Braintrust, or logs; output still passes secret redaction because hostile code may try to disclose what it receives.
...
Candidate creation has three entry paths: a Fireworks plan, a deterministic native quick fix for rules with one unambiguous semantic operation, and a user-authored semantic operation through the review UI. All three pass the same schema, evidence, capability, policy, materialization, and validation gates.
...
Braintrust receives pseudonymous IDs, rule/adapter versions, model/template settings, token and latency data, schema/guard outcomes, candidate status, and a linked terminal validation summary class. It does not receive raw events, Daytona phase telemetry, sandbox health, cleanup details, plaintext secrets, private model reasoning, or full proprietary source. Queue, Worker, database, GitHub, and Daytona operational telemetry stays in Cloudflare logs/metrics plus Neon/R2.
...
type ValidationJobResult =
  ...
  | (ValidationJobResultBase & {
      ...
      precedingOutcome: "passed" | NonCleanupFailureOutcome;
      attestationDigest?: never;
    });
```
...
type SourceInput =
  ...
  | {
      ...
      secretScanPolicyVersion: string;
      submoduleIdentities: SourceComponentIdentity[];
      lfsIdentities: SourceComponentIdentity[];
      supportGapIds: string[];
    };
...
5. Run post-materialization guards: path scope, secret scan, diff-size policy, semantic reparse, manifest-lock coherence, generated-file classification, policy checks, and operation-to-diff traceability.
...
If the unchanged baseline already passes the behavior a candidate claims to fix, the result cannot be called a reproduced fix. It may become a separately labelled validated hardening or declaration-consistency recommendation if deterministic evidence and all required gates support that narrower claim.
...
For committed source, the backend obtains a short-lived, one-repository GitHub token, fetches an archive using an authorization header, and streams or uploads a content-addressed bundle. For an eligible dirty working tree, the Companion creates an exact patch plus allowed untracked-file bundle against the recorded base commit, applies ignore/size/secret policy locally, encrypts it, and uploads it as a content-addressed source object. If policy blocks a required file, validation waits for commit or explicit policy resolution; it never substitutes the older commit. Daytona never receives the GitHub App private key or a broad GitHub token.
...
interface ValidationJobKey {
  ...
  secretBindingSchemaDigest?: string;
}
...
- user sign-in and product session creation;
...
User authentication uses OAuth state, PKCE, exact callback validation, encrypted server-side user/refresh tokens, and an `HttpOnly`, `Secure`, `SameSite` product-session cookie. Repository selection uses the authenticated user's GitHub App installation/repository intersection rather than every repository visible to the installation.

The first authenticated login creates one personal workspace by default. Its owner may later create a one-time invite for another authenticated GitHub user; repository access and workspace membership remain separate checks.

Device pairing is a one-time, short-lived, workspace-bound exchange tied to a device-generated public signing key. The extension receives only a revocable product device credential.

Minimum repository permissions:

| Operation | Token and downscoped permission |
|---|---|
| Source archive/read | One-repository installation token with `Contents: read` |
| Approved branch/commit | User token where possible, or one-repository installation token with `Contents: write` |
| Pull request | User token where possible, or one-repository installation token with `Pull requests: write` |
| Alpha check run | One-repository installation token with `Checks: write` |

`.github/workflows/**` remains a separate explicit permission/approval path; otherwise the product exports a manual patch. GitHub webhook processing returns quickly after validation and queueing. Repository numeric IDs are canonical; names are display values. Installation tokens are created just in time with `repository_ids: [repository_id]` and reduced permissions, and never persisted beyond encrypted short-lived cache metadata.
...
5. A trusted Daytona bootstrap generates its own ephemeral public key and exchanges a one-time object-access token for the object key sealed to that key. Only the per-object key reaches that sandbox; the server master key never does.

Direct R2 upload/download authorization is shortest-practical and bucket/object scoped. Presigned URLs and one-time object-access tokens are bearer credentials: they are never persisted in Workflow state, logs, Braintrust, Fireworks, command arguments, or cache keys.
...
All list APIs use opaque cursors. Mutation APIs require an idempotency key. Device ingestion uses enrolled device authentication; browser APIs use product sessions; webhook routes use provider signatures. A trusted Daytona bootstrap authenticates a download with a one-time, validation-job-bound bootstrap token and submits an ephemeral X25519 public key. The Worker verifies the job, workspace, object, target, expiry, and single-use binding before returning shortest-practical object download authorization and the per-object key sealed to that public key; it never returns the server master key.

## Correctness Properties

Each property must become an automated invariant, property-based test, or release-gate assertion.

1. **Local secret boundary.** No plaintext seeded secret crosses from observation into durable SQLite event payloads, uploads, R2, Neon, Fireworks, Braintrust, or logs.  
...
6. **Realm isolation.** Evidence from one realm or dependency layer never proves a change in another.  
...
15. **LLM non-authority.** Fireworks output alone cannot create a confirmed finding, attribution, validation pass, or verified label.  
...
21. **Sandbox separation.** Final validation never reuses materialization state or another candidate's installed state.  
    _Validates: Requirements 14.3, 14.7, 16.5_
...
24. **Required-target completeness.** Verified requires a pass for every effective required target.  
    _Validates: Requirements 14.6_

25. **Behavioral proof.** Clean install without an accepted behavior contract produces at most `reconstruction_passed`.  
...
35. **Baseline comparability.** Every candidate is evaluated beside an unchanged baseline with the same Source_Input, target, immutable base, and behavior contract; a passing baseline prevents a false “reproduced and fixed” claim.  
...
37. **Secret-binding restraint.** A job receives only an allowed opaque, target/host-scoped secret binding; arbitrary raw-secret access remains blocked and can never be silently enabled.  
    _Validates: Requirements 20.7_
...
| Local disk is full | Drop bulky optional payloads, preserve a minimal capture-gap record, and notify the user |
...
| Required just-in-time secret is unavailable | Return `security_blocked`; never weaken the behavior contract |
...
- Generators cover event ordering, overlapping actions, graph identities, action attribution, policies, cache inputs, target matrices, validation outcomes, secret formats, and source-binding changes.
...
Scores include structured-output validity, evidence-citation precision, unsupported-operation rate, candidate minimality, guard-rejection reason, and secret leakage. Guarded hallucinations must never become executable candidates.
...
- seeded secrets at every input and output boundary.
...
3. The primary fixture performs:
   agent-observed undeclared dependency use → deterministic finding → Fireworks candidate → native patch → clean Daytona pass → verified recommendation → GitHub pull request.
4. A negative fixture injects Daytona/registry failure and proves that no dependency recommendation is created.
5. A privacy fixture seeds credentials in environment, commands, output, and files and proves no plaintext cloud persistence.
...
- Daytona, Fireworks, Braintrust, GitHub, database, and encryption credentials are Worker secrets.
...
1. Schema generation and TypeScript/Rust contract checks pass.
2. TypeScript, Rust, deterministic rule, and MVP adapter conformance suites pass.
3. Database migration runs successfully on a disposable database.
4. Seeded-secret corpus produces zero plaintext persistence.
...
9. Preview deployment passes the Playwright vertical slice.
...
- passed, failed, inconclusive, infrastructure, and unsupported ratios;
