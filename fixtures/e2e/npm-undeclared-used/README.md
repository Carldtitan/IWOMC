...
from the vendored package. The mutable environment then passes while a clean `npm ci` followed by
`npm test` fails.

`expected/` contains the conservative native-manager result: the vendored package is added as one
direct dependency, npm generates the lockfile, and the same behavior test passes from clean state.
