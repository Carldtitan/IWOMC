...
- **Verified**: passed every required clean validation target and policy gate.
...
3. Deterministic evidence and native tooling MUST decide whether a disagreement exists and whether validation passed. An LLM MUST NOT make either decision.
4. Fireworks MAY explain evidence and generate structured candidate patches, but every candidate MUST pass schema, semantic, policy, and Daytona validation gates.
...
9. No repository change may be applied without an authorized user action or an explicit, versioned workspace approval policy.
...
- **Recommendation**: A candidate shown to a user. Only a candidate satisfying every required proof gate may be labelled verified.
...
1.1. WHEN a user opens a linked repository in VS Code or Cursor, THE Environment_Reconciler SHALL observe the linked local environment without replacing the IDE, terminal, coding agent, or package manager.
...
1.5. WHEN the user has not approved a change, THE Environment_Reconciler SHALL leave the repository and host environment unchanged.
...
**User Story:** As a new user, I want a short and understandable setup flow, so that my repository, local device, and team workspace are connected with least privilege.

#### Acceptance Criteria

2.1. WHEN a user selects **Sign in with GitHub**, THE Environment_Reconciler SHALL authenticate through the product's GitHub App and store the immutable GitHub user identifier.

2.2. WHEN repository access is required, THE Environment_Reconciler SHALL request access only through a selected GitHub App installation and SHALL list only repositories available to that installation and user.

2.3. WHEN a user creates a workspace, THE Environment_Reconciler SHALL create a personal workspace by default and SHALL allow the user to invite collaborators later.

2.4. WHEN a user links a repository, THE Environment_Reconciler SHALL record the repository ID, default branch, installation ID, default policy, and initial behavior-contract discovery state.

2.5. WHEN the extension is not paired, THE Environment_Reconciler SHALL expose a single **Connect workspace** action that opens a browser, authenticates the user, and returns a short-lived device-pairing result to the extension.

2.6. THE extension SHALL receive a product session credential, never a GitHub App private key, GitHub client secret, GitHub refresh token, Daytona key, Fireworks key, Braintrust key, or database credential.
...
4.3. WHEN an observed session ends or a user selects **Scan now**, THE Companion SHALL create an incremental checkpoint inventory. WHEN a pull request changes relevant files, THE backend SHALL request a checkpoint from an authorized online device in the applicable realm; IF none is online, THEN it SHALL use only repository evidence plus the latest inventory and create an explicit stale-device capture gap.

4.4. AN inventory SHALL be structured and scoped; it SHALL NOT copy a full disk image or indiscriminately upload a user's machine state.
...
7.6. THE private alpha SHALL promote Poetry, Conda-family tools, pnpm, Yarn, Bun, Cargo, CMake, and at least one of vcpkg or Conan only after each adapter passes its native conformance suite.
...
8.3. THE declared graph SHALL be derived from semantic manifests, runtime selectors, setup files, CI, containers, and user-confirmed behavior policy.
...
10.2. THE user SHALL be able to review, edit, order, enable, and disable discovered commands before they become the required behavior contract.

10.3. EACH behavior-contract command SHALL include its working directory, realm/target applicability, timeout, required secrets by reference, expected exit status, and artifact or endpoint assertions when applicable.

10.4. IF no behavior contract has been accepted, THEN a clean install MAY be labelled `reconstruction_passed` but the candidate SHALL NOT be labelled a verified recommendation.
...
11.8. THE product SHALL use its own Fireworks account by default. End users SHALL NOT need to provide a model API key for the core service.
...
12.2. BEFORE Daytona execution, EACH candidate SHALL pass schema validation, path-scope validation, semantic parse validation, policy validation, and static contradiction checks.
...
12.5. A removal candidate SHALL remain unverified until an isolated ablation run removes the dependency and passes every applicable behavior-contract command on every required target.
...
### Requirement 13: Default and user-defined optimality (MVP foundation; Alpha completion)
...
13.4. THE alpha product SHALL allow authorized users to define versioned constraints and weighted objectives for install time, build time, runtime latency, memory, disk, image size, dependency count, version freshness, license, or security policy.

13.5. WHEN objectives conflict and no user priority resolves the conflict, THE Environment_Reconciler SHALL show the non-dominated candidates and their measurements rather than claim one universal optimum.
...
14.1. THE validation planner SHALL derive required targets from workspace policy, repository configuration, CI matrices, runtime selectors, and user-confirmed targets.
...
14.7. A Daytona fork or warmed sandbox MAY accelerate exploratory work, but final proof SHALL use clean reconstruction with no candidate-installed state inherited from another job.
...
15.2. THE validator SHALL classify terminal outcomes as `passed`, `project_or_candidate_failed`, `infrastructure_failed`, `resource_budget_failed`, `timed_out`, `security_blocked`, `unsupported_target_or_capability`, `inconclusive`, or `cleanup_failed`.
...
**User Story:** As a user paying for validation, I want the product to finish quickly without allowing caches to hide undeclared state, so that speed does not weaken proof.
...
16.4. A validation cache key SHALL include repository commit or content identity, candidate patch hash, target/base identity, behavior contract, policy, adapter/rule versions, manager/tool versions, secret schema version where relevant, and every input capable of changing the decision.
...
17.3. WHEN an authorized user selects **Apply via pull request**, THE backend SHALL create a product branch from the verified Source_Input base, materialize the exact approved Source_Input changes, apply the exact verified candidate patch, verify the resulting tree digest, and open or update a reviewable GitHub pull request. IF the Source_Input contains uncommitted changes, THE user SHALL see and explicitly approve those source changes separately from the candidate patch.

17.4. THE MVP SHALL NOT write directly to the default branch.

17.5. THE GitHub App SHALL use the least-privileged installation or user token applicable to each clone, check, branch, and pull-request action.
...
18.2. THE extension SHALL perform normal observation and upload in the background and SHALL notify the user only for actionable findings, degraded coverage, required approval, or terminal validation outcomes.
...
18.8. THE UI SHALL expose support and capture gaps prominently enough that a user cannot confuse partial coverage with a clean environment.
...
19.1. EACH Fireworks request SHALL create a Braintrust trace containing project-safe identifiers, finding category, prompt/template version, model and sampling configuration, latency, token/cost data when available, structured-output validity, tool/adapter versions, candidate outcome, and final validation class.

19.2. BRAINTRUST SHALL NOT receive raw secrets, environment-variable values, unredacted terminal output, full raw conversations, or unnecessary proprietary source.

19.3. THE product SHALL maintain a versioned evaluation set covering valid candidate generation, unsupported-operation refusal, evidence citation, minimal-change behavior, secret blocking, and failure recovery.

19.4. A model or prompt version SHALL NOT become the default until it passes the configured evaluation gates.
...
**User Story:** As a developer, I want the system to prove environment changes without collecting my credentials or private conversations, so that installing it does not create a larger security problem.
...
20.3. FOR relevant environment variables, THE Companion MAY retain only name, presence, scope/source, changed state, secret-detection result, and a keyed HMAC equality fingerprint when comparison is necessary.

20.4. THE HMAC key SHALL remain in the operating-system credential store, SHALL be versioned and rotatable, and SHALL NOT be uploaded with its fingerprints.

20.5. THE redactor SHALL cover supported credential formats, private keys, tokens, connection strings, URL credentials, registry credentials, `.env` values, command arguments, process output, provider payloads, paths, internal hosts, and user-defined patterns.
...
20.7. WHEN the MVP encounters a private dependency or behavior command that requires a secret, THE validator SHALL return `security_blocked` rather than request or persist the raw value. IN private alpha, THE validator MAY support an existing opaque secret reference only when the configured provider can enforce the required host/target scope and lifetime. Product-controlled plumbing SHALL keep the resolved value out of prompts, cache keys, command-line arguments, workflow state, logs, artifacts, and analytics, and sandbox output SHALL still pass local/server redaction. Arbitrary raw-secret access by untrusted project code SHALL remain `security_blocked`.
...
20.10. A privacy conformance corpus containing supported secret formats SHALL produce zero plaintext secret persistence before public release.

20.11. AN exact Source_Input bundle authorized for Daytona validation is distinct from optional observational raw content, but it SHALL be secret-scanned locally, encrypted with a short retention class, restricted to the applicable workspace and validation jobs, and excluded from Fireworks and Braintrust payloads.
...
8. produce a verified recommendation only for a passing candidate; and
...
22.4. THE conformance suite SHALL seed representative API keys, passwords, tokens, connection strings, private keys, registry credentials, and `.env` values and SHALL assert that no plaintext value reaches the spool, backend, object storage, logs, Fireworks, Braintrust, or validation artifacts.

22.5. NATIVE adapters SHALL pass fixtures for parsing, graphing, precedence, lock verification, semantic editing, clean install, cache equivalence, and malformed or unsupported inputs before being marked `full_native`.
...
22.7. A release SHALL be blocked by a false verified recommendation, plaintext secret persistence, an unbounded retry loop, an unhandled orphaned sandbox, a migration incompatibility, or a core adapter conformance failure.

## Target Ecosystem Registry Seed

The registry below preserves the target-product coverage discovered during requirements research. It is not an MVP claim. Every row begins as `observed_only` or `unsupported` and is promoted per manager, version, operation, platform, and Daytona target only after native conformance passes.
...
Each surface has its own capability profile; success on one surface does not imply success on another.
...
4. Daytona can demonstrate that the unchanged repository fails clean reconstruction while the candidate passes the accepted behavior contract.
...
7. The end-to-end flow works without collecting plaintext secrets or requiring developers to code in Daytona.
...
| Provide default and user-defined optimality | R13 |
...
| Reduce secrets and avoid raw environment storage | R20 |
